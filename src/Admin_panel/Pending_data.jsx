    ...
    <div>
       ...
       <span className={`py-2 px-3 bg-white me-2 ${value==='university' ? 'fw-medium':''}`} onClick={()=>{
               ...
               <span className={`py-2 px-3 bg-white me-2 ${value==='courses' ? 'fw-medium':''}`} onClick={()=>{
                setvalue("courses")
                fetchCoursedata()
               }}  style={{fontSize:'16px',cursor:'pointer',color:value==='courses' ? '#5d5fe3':'gray',borderBottom:value==='courses' ? '2px solid #5d5fe3':'none'}}>Course</span>
               <span onClick={()=>{
                setvalue("groups")
                fetchGroupdata()
...
<h6 className={`text-secondary text-center py-4 ${pending_report.length>0 ? 'd-none':''}`}>No Pending University reports available...💬</h6>
       <div class={`table-responsive mt-3 rounded ${pending_report.length>0 ? '':'d-none'}`}>
  ...
  <tbody>
    {pending_report.map((x,index)=>{
     ...
     }} style={{cursor:'pointer'}}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check-square-fill" viewBox="0 0 16 16">
  <path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zm10.03 4.97a.75.75 0 0 1 .011 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.75.75 0 0 1 1.08-.022z"/>
</svg></span>

<span onClick={()=>{
        accept_or_reject('False',x.university_id)
     }} style={{cursor:'pointer'}} className='ms-0 ms-md-2'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="red" class="bi bi-x-square-fill" viewBox="0 0 16 16">
...
<h6 className={`text-secondary text-center py-4 ${pending_course_report.length>0 ? 'd-none':''}`}>No Pending Course reports available...💬</h6>
       <div class={`table-responsive mt-3 rounded ${pending_course_report.length>0 ? '':'d-none'}`}>
       <div className='d-flex justify-content-end align-items-center pb-3'>
  ...
  <thead>
    <tr>
      ...
      <th scope="col" className='fw-medium text-secondary' style={{fontSize:'15px'}}>Requested by</th>
      <th scope="col" className={`fw-medium text-secondary`} style={{fontSize:'15px'}}>University Name</th>
      <th scope="col" className={`fw-medium text-secondary`} style={{fontSize:'15px'}}>Course Name</th>
      <th></th>
    </tr>
  </thead>
  <tbody>
...
<h6 className={`text-secondary text-center py-4 ${pending_group_report.length>0 ? 'd-none':''}`}>No Pending Group reports available...💬</h6>
       <div class={`table-responsive mt-3 rounded ${pending_group_report.length>0 ? '':'d-none'}`}>
       <div className='d-flex justify-content-end align-items-center pb-3'>
...
<h6 className={`text-secondary text-center py-4 ${pending_program_report.length>0 ? 'd-none':''}`}>No Pending Program reports available...💬</h6>
       <div class={`table-responsive mt-3 rounded ${pending_program_report.length>0 ? '':'d-none'}`}>
       <div className='d-flex justify-content-end align-items-center pb-3'>
