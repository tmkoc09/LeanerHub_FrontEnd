...
const Navbar = ({setLanguage,count2,count,setindex1,setCount}) => {
  ...
  const handleLogout=()=>{
    axios.post(`${ipaddress}/User_Logout/${userdata.user_id}/`)
    .then((r)=>{
...
const pushnotificationcontrol=()=>{
  axios.get(`${ipaddress}/PushNotificationsControl/${user.user_id}/`)
  .then((r)=>{
...
const[userdetails,setUserdetails]=useState({})
  ...
  .catch(()=>{
    console.log("User Details Fetching Error")
        ...
        axios.get(`${ipaddress}/user_total_credits/${user.user_id}/`)
        .then((r)=>{
        ...
        axios.get(`${ipaddress}/displaynotifications/${user.user_id}/`)
        .then((r)=>{
        ...
        axios.get(`${ipaddress}/PushNotifications/${user.user_id}/`)
        .then((r)=>{
      ...
      // To get the Joined courses of user
        axios.get(`${ipaddress}/CoursesView/${user.user_id}/`)
        .then((r) => {
...
const deleteNotification=(id)=>{
  axios.delete(`${ipaddress}/deletenotification/${id}/`)
  .then((r)=>{
    setCount1(count1+1)
  })
  .catch(()=>{
...
const deletepushNotification=(id)=>{
  axios.delete(`${ipaddress}/PushNotificationsDelete/${id}/`)
  .then((r)=>{
    setCount1(count1+1)
  })
  .catch(()=>{
...
const rejectRequest=(user_id,id)=>{
  axios.delete(`${ipaddress}/groupadminacceptance/${user_id}/${id}/`)
  .then((r)=>{
    setCount1(count1+1)
  })
  .catch(()=>{
...
const clearall=()=>{
  ...
  return (
    <div>
      <nav class="navbar navbar-expand-lg navbar-expand-md bg-light py-3 fixed-top d-none d-lg-block  animate__animated animate__fadeIn" style={{zIndex:3}}>
        <div class="container-fluid">

            <div className="d-flex w-100 ps-lg-5">
              <div class="ps-5 d-flex navbardiv ms-5 align-items-center">
                <div class="input-group navbar-input" style={{cursor:'pointer',height:'45px'}} type="button" data-bs-toggle="offcanvas" data-bs-target="#searchoffcanvas" aria-controls="searchoffcanvas">
                <span class="input-group-text bg-white border-end-0" style={{color:'#AAB0B0'}} id="basic-addon1"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
<path d="M20.2965 20.9936L13.774 14.4712C13.2532 14.9145 12.6542 15.2577 11.9772 15.5008C11.3001 15.7439 10.6197 15.8654 9.93589 15.8654C8.26749 15.8654 6.85546 15.2879 5.69982 14.1329C4.54416 12.9778 3.96633 11.5666 3.96633 9.89908C3.96633 8.23158 4.54384 6.81924 5.69886 5.66205C6.85387 4.50486 8.26513 3.92627 9.93263 3.92627C11.6001 3.92627 13.0125 4.50409 14.1697 5.65973C15.3268 6.81538 15.9054 8.22741 15.9054 9.89583C15.9054 10.6196 15.7772 11.3201 15.5208 11.9972C15.2644 12.6743 14.9279 13.2532 14.5112 13.734L21.0337 20.2564L20.2965 20.9936ZM9.93589 14.8237C11.3181 14.8237 12.485 14.348 13.4365 13.3964C14.388 12.4449 14.8638 11.278 14.8638 9.89583C14.8638 8.51362 14.388 7.34676 13.4365 6.39523C12.485 5.4437 11.3181 4.96794 9.93589 4.96794C8.55368 4.96794 7.38682 5.4437 6.43529 6.39523C5.48378 7.34676 5.00802 8.51362 5.00802 9.89583C5.00802 11.278 5.48378 12.4449 6.43529 13.3964C7.38682 14.348 8.55368 14.8237 9.93589 14.8237Z" fill="#8E9696"/>
  ...
  <input type="text" style={{cursor:'pointer'}} class="form-control nav-input ps-0 border-start-0" placeholder={translate_value.navbar.search} aria-label="Username" aria-describedby="basic-addon1"/>
</div>
                <button onClick={()=>{
                  navigate(`/uploadpage/0/""`)
                }} className="btn ms-3 text-white navbar-btn" style={{backgroundColor:'#5D5FE3',width:'150px',height:'45px'}}><i class="fa-solid fa-plus me-2"></i>{translate_value.navbar.add}</button>
              </div>

              <ul class="navbar-nav gap-4 d-flex align-items-center justify-content-end" style={{width:'40%'}}>
              <li class="nav-item">
...
{/* -----------------------------------------------------Rewards icon-------------------------------------------------- */}
                <li class="nav-item d-flex align-items-center">
                  <Link to='/rewards'  class="nav-link d-flex align-items-center">
...
<li class="nav-item" style={{position:'relative'}}>
                  <a class="nav-link d-flex align-items-center" type="button" data-bs-toggle="offcanvas" data-bs-target="#notification" aria-controls="notification">
                  <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 35 35" fill="none">
...
<circle cx="2.5" cy="2.5" r="2.5" fill="#FF845D"/>
   ...
   {userdetails.nickname!=undefined ? ( <p className={userdetails.profile_pic ==null ? 'd-flex bg-success text-white rounded-circle my-auto justify-content-center align-items-center' : 'd-none'} style={{fontSize:'16px',width:'40px',height:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
		    ...
		    <ul className={`border-0 shadow px-2 bg-white p-3 px-4 rounded ${navbar_dropdown_visible ? '':'d-none'}`} style={{position:'absolute',top:'60px',listStyleType:'none',left:'-220px'}}>
			  <li className="ps-1"><Link class="dropdown-item mb-3" to={`/profile/${user.user_id}`}  style={{fontFamily:" 'DM Sans', sans-serif",fontSize:'16px',lineHeight:'28px',color:'#2A3941'}}><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
  <path d="M9 8.23075C7.96875 8.23075 7.08594 7.86356 6.35156 7.12918C5.61719 6.39481 5.25 5.512 5.25 4.48075C5.25 3.44947 5.61719 2.56665 6.35156 1.83228C7.08594 1.0979 7.96875 0.730713 9 0.730713C10.0312 0.730713 10.9141 1.0979 11.6484 1.83228C12.3828 2.56665 12.75 3.44947 12.75 4.48075C12.75 5.512 12.3828 6.39481 11.6484 7.12918C10.9141 7.86356 10.0312 8.23075 9 8.23075ZM0.25 17.2692V15.2115C0.25 14.6955 0.40024 14.2127 0.700719 13.7632C1.0012 13.3137 1.40545 12.9647 1.91347 12.7163C3.09295 12.1506 4.27323 11.7263 5.45431 11.4435C6.63542 11.1606 7.81731 11.0192 9 11.0192C10.1827 11.0192 11.3646 11.1606 12.5457 11.4435C13.7268 11.7263 14.9071 12.1506 16.0865 12.7163C16.5946 12.9647 16.9988 13.3137 17.2993 13.7632C17.5998 14.2127 17.75 14.6955 17.75 15.2115V17.2692H0.25ZM1.5 16.0192H16.5V15.2115C16.5 14.9343 16.4107 14.6738 16.232 14.4302C16.0533 14.1867 15.8061 13.9807 15.4904 13.8125C14.4615 13.3141 13.4008 12.9323 12.308 12.667C11.2153 12.4018 10.1126 12.2692 9 12.2692C7.8874 12.2692 6.78473 12.4018 5.692 12.667C4.59925 12.9323 3.53846 13.3141 2.50963 13.8125C2.19392 13.9807 1.94672 14.1867 1.76803 14.4302C1.58934 14.6738 1.5 14.9343 1.5 15.2115V16.0192ZM9 6.98075C9.6875 6.98075 10.276 6.73595 10.7656 6.24637C11.2552 5.75679 11.5 5.16825 11.5 4.48075C11.5 3.79325 11.2552 3.2047 10.7656 2.71512C10.276 2.22554 9.6875 1.98075 9 1.98075C8.3125 1.98075 7.72396 2.22554 7.23438 2.71512C6.74479 3.2047 6.5 3.79325 6.5 4.48075C6.5 5.16825 6.74479 5.75679 7.23438 6.24637C7.72396 6.73595 8.3125 6.98075 9 6.98075Z" fill="#2A3941"/>
			  ...
			  <li className=""><Link class="dropdown-item mb-3"  style={{fontFamily:" 'DM Sans', sans-serif",fontSize:'16px',lineHeight:'28px',color:'#2A3941'}}><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
...
<li className=""><Link data-bs-toggle="modal" data-bs-target="#need_help_modal" class="dropdown-item mb-3"  style={{fontFamily:" 'DM Sans', sans-serif",fontSize:'16px',lineHeight:'28px',color:'#2A3941'}}><svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 35 35" fill="none">
  ...
  <path d="M15 15.1442L5 8.60578V21.7308C5 21.9551 5.07211 22.1394 5.21634 22.2837C5.36057 22.4279 5.54487 22.5 5.76922 22.5H16.875V23.75H5.76922C5.19391 23.75 4.71354 23.5573 4.32813 23.1719C3.94271 22.7865 3.75 22.3061 3.75 21.7308V8.26922C3.75 7.69391 3.94271 7.21354 4.32813 6.82812C4.71354 6.44271 5.19391 6.25 5.76922 6.25H24.2308C24.8061 6.25 25.2865 6.44271 25.6719 6.82812C26.0573 7.21354 26.25 7.69391 26.25 8.26922V16.875H25V8.60578L15 15.1442ZM15 13.75L24.6154 7.5H5.38463L15 13.75ZM23.2692 27.2356L22.4087 26.375L24.9783 23.75H18.8942V22.5H25.0024L22.3774 19.875L23.2692 19.0144L27.3798 23.125L23.2692 27.2356ZM5 8.60578V23.3413V16.875V17.0889V7.5V8.60578Z" fill="#2A3941"/>
			  ...
			  <li className=""><a onClick={handleLogout} class="dropdown-item" style={{fontFamily:" 'DM Sans', sans-serif",fontSize:'16px',lineHeight:'28px',color:'#2A3941'}}><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
...
{/* navbar for Small screens */}
      ...
      <nav class="navbar navbar2 navbar-expand bg-white py-2 fixed-top border-bottom d-block d-lg-none">
  <div class="container-fluid">
    <button
      class="border-0"
      type="button"
    ...
    <ul class="navbar-nav">
    <li class="nav-item d-flex align-items-center">
                  <Link to='/rewards'  class="nav-link mx-2 text-uppercase">
  ...
  <path d="M14.4423 29.1058L15.6923 20.5H10.3798L19.7308 7.0144H20.3077L19.0817 16.75H25.3317L15.0192 29.1058H14.4423Z" fill="white"/>
    ...
    <li class="nav-item dropdown">
                  <a class="nav-link mx-2 d-flex align-items-center" type="button" data-bs-toggle="offcanvas" data-bs-target="#notification" aria-controls="notification">
                  ...
                  <ul class={notification1<=0 && notification2<=0 && pushNotification.length<=0 ? "d-none" : "dropdown-menu fade-up dropdown-menu-end"} style={{width:'max-content'}} aria-labelledby="notificationDropdown">
                    {notification1 && (
  notification1.map((x)=>{
    return(
        <li className="d-flex justify-content-between align-items-center px-2 mb-2">
          <a class="text-secondary" style={{textDecoration:'none',fontSize:'13px'}}>{x ? `${x.user_data.profile_pic.nickname} from ${x.university_name} requested for group ${x.group_name}` : "No Notifications"}</a>
  ...
  notification2.map((x)=>{
    return(
        <li className="d-flex justify-content-between align-items-center px-2 py-2">
          <Link class="text-secondary text-decoration-none" style={{textDecoration:'none',fontSize:'13px'}}>{x.message}</Link>
...
{pushNotification &&(
  pushNotification.map((z)=>{
    return(
        <li className="d-flex justify-content-between align-items-center px-2 py-2">
          <a class="text-secondary" style={{textDecoration:'none',fontSize:'13px'}}><span>{z.notification}</span></a>
         <button className="btn btn-sm btn-transparent ms-2" onClick={()=>{
...
<div
        class="offcanvas offcanvas-start d-sm-block d-lg-none d-xl-none"
      ...
      >
        <div class="offcanvas-header d-flex justify-content-between align-items-center">
          ...
          <svg data-bs-dismiss="offcanvas" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293z"/>
</svg>
        </div>
        <div class="offcanvas-body">
          <div class="pb-5">
          <ul class="nav flex-column gap-3 text-start ps-0 pb-4">
  <li class="nav-item">
    <Link to='/dashboard/page' class="nav-link"><span className="nav-icon ms-1"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 25 25" fill="none">
  ...
  <li class="nav-item">
    <a data-bs-toggle="offcanvas"
      ...
      aria-controls="courseOffcanvas" class="nav-link" type="button"><span className="nav-icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="26" viewBox="0 0 35 35" fill="none">
  ...
  <li class="nav-item">
    <Link data-bs-toggle="offcanvas"
      ...
      aria-controls="groupOffcanvas"  class="nav-link" type="button"><span className="nav-icon"><svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 35 35" fill="none">
  ...
  <li class="nav-item">
    <Link  data-bs-toggle="offcanvas"
      ...
      aria-controls="studylistcanvas" class="nav-link"><span className="nav-icon"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 35 35" fill="none">
...
<li class="nav-item">
    <Link to={`/profile/${user.user_id}`} class="nav-link"><span className="nav-icon"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
  ...
  <li class="nav-item">
    <p onClick={()=>{
      handleLogout()
    }} style={{cursor:'pointer'}} class="nav-link"><span className="nav-icon"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
    <path d="M7.01922 25C6.44391 25 5.96354 24.8073 5.57812 24.4219C5.19271 24.0365 5 23.5561 5 22.9808V7.01922C5 6.44391 5.19271 5.96354 5.57812 5.57812C5.96354 5.19271 6.44391 5 7.01922 5H15.024V6.25H7.01922C6.82693 6.25 6.65065 6.33013 6.49038 6.49038C6.33013 6.65065 6.25 6.82693 6.25 7.01922V22.9808C6.25 23.1731 6.33013 23.3494 6.49038 23.5096C6.65065 23.6699 6.82693 23.75 7.01922 23.75H15.024V25H7.01922ZM20.5769 19.4231L19.6995 18.524L22.5986 15.625H11.4904V14.375H22.5986L19.6995 11.476L20.5769 10.5769L25 15L20.5769 19.4231Z" fill="#2A3941"/>
  </svg></span>
...
<div class="offcanvas offcanvas-end border-0 shadow" data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1" id="notification" aria-labelledby="notificationLabel" style={{marginTop:'74px'}}>
  <div class="offcanvas-header py-4" style={{backgroundColor:'#F3F0FF'}}>
    <h5 class="offcanvas-title d-flex align-items-center" id="offcanvasScrollingLabel" style={{fontSize:'16px'}}>Notifications<span className={notification1.length>0 || notification2.length>0 || pushNotification.length>0 ? "d-block ms-1 rounded-circle p-1 px-2 text-white" : "d-none"} style={{fontSize:'11px',backgroundColor:'#FF845D'}}> {notification1.length+notification2.length+pushNotification.length}</span></h5>
    <button data-bs-dismiss="offcanvas" aria-label="Close" className='bg-transparent border-0'><i class="fa-solid fa-circle-xmark fs-5 text-secondary"></i></button>
  </div>
  <div class="offcanvas-body p-0 px-2">

  <div class={notification1<=0 && notification2<=0 && pushNotification.length<=0 ? "d-none" : "d-block text-end mt-1"}>
      <span onClick={()=>{
        clearall()
      }} style={{color:'#5d5fe3',fontSize:'13px',textDecoration:'underline',cursor:'pointer'}}>Clear All</span>
    </div>
    <div class={notification1<=0 && notification2<=0 && pushNotification.length<=0 ? "d-block text-center mt-4" : "d-none"}>
      <p className="fw-medium text-secondary">No Notifications !!!!!</p>
    </div>
  <ul class={notification1<=0 && notification2<=0 && pushNotification.length<=0 ? "d-none" : "p-0"} style={{listStyleType:'none'}} aria-labelledby="notificationDropdown">
...
<div className="ms-2 d-flex">
<Link to={`/groupchat/privategroup/${x.group_id}`} class="text-secondary my-auto" style={{textDecoration:'none',fontSize:'13px'}}>{x.message}</Link>
      <div className="d-flex gap-2 align-items-center ms-3">
      <button className="btn btn-sm btn-transparent ms-2" onClick={()=>{
          deleteNotification(x.id)
         }}><i class="fa-solid fa-xmark"></i></button>
...
pushNotification.map((z)=>{
  return(
   <li className="d-flex justify-content-between align-items-center px-1 py-3 border-bottom">
    ...
    <div className="ms-2">
    <a class="" style={{textDecoration:'none',fontSize:'14px',color:'#37454D'}}>{z.notification}</a>
