...
const fetchuploaded_docs=()=>{
  axios.get(`${ipaddress}/uploadDocuments/${user_id}/`)
  .then((r)=>{
...
const fetchfollowed_docs=()=>{
  axios.get(`${ipaddress}/FavouriteDocuments/${user_id}`)
  .then((p)=>{
...
const fetchcreated_flashsets=()=>{
  axios.get(`${ipaddress2}/UserFlashcardsetsRetrive/${user_id}/`)
  .then((p)=>{
    const docs=r.data.reverse()
...
const fetchsaved_flashsets=()=>{
  axios.post(`${ipaddress2}/SavedStudylistFlashsets/`,{
    'user_id':user_id
  })
  .then((p)=>{
    const docs=p.data.reverse()
...
const filterfolloweddocs=(value)=>{
  if(value==="followeddocs_original"){
    ...
    axios.get(`${ipaddress}/FavouriteDocumentsFilter/${user_id}/${value}/`)
    .then((r)=>{
...
const searchDocument=(document_name)=>{
  ...
  axios.get(`${ipaddress}/UserDocumentsSearch/${user.user_id}/${document_name}/`)
  .then((r)=>{
...
const search_followeddocs=(value)=>{
  ...
  axios.get(`${ipaddress}/FavouriteDocumentsSearchFilter/${user.user_id}/${value}/`)
  .then((r)=>{
...
if(value.length>0){
  axios.get(`${ipaddress2}/SearchFlashsetsCreatedFlashsets/${user.user_id}/${value}/`)
  .then((r)=>{
...
const searchSavedflashsets=(value)=>{
  if(value.length>0){
    axios.get(`${ipaddress2}/SearchFlashsetsSavedFlashsets/${user.user_id}/${value}/`)
    .then((r)=>{
...
const filtercreatedflashsets=(value)=>{
  if(value==="likes"){
  axios.post(`${ipaddress2}/FlashsetsFiltersCreatedFlashsets/${user.user_id}/`,{
    'like':true,
    'datetime':false})
  .then((r)=>{
    console.log("Liked")
    console.log("date")
    console.log("Created Flashsets Filtered Successfuly",r.data)
    setcreatedFlashsets(r.data)
...
const filtersavedflashsets=(value)=>{
  if(value==="likes"){
  axios.post(`${ipaddress2}/FlashsetsFiltersSavedFlashsets/${user.user_id}/`,{
    'like':true,
    'datetime':false})
  .then((r)=>{
    console.log("Liked")
    console.log("date")
    console.log("Saved Flashsets Filtered Successfuly",r.data)
  ...
  return (
    <div>
      ...
      <Mainsidebar></Mainsidebar>
        ...
        <Link to={`/profile/${user_id}`}><svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="black" class="bi bi-arrow-left" viewBox="0 0 16 16">
  <path fill-rule="evenodd" d="M15 8a.5.5 0 0 0-.5-.5H2.707l3.147-3.146a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L2.707 8.5H14.5A.5.5 0 0 0 15 8"/>
</svg></Link> 
          <ul class="nav nav-underline mt-4 d-flex justify-content-center" id="myTab" role="tablist">
  <li class="nav-item" role="presentation">
    <button class={`nav-link bg-white me-lg-4 ${value==="uploaded_documents" ? 'text-primary active' : 'text-secondary'}`} id="home-tab" data-bs-toggle="tab" data-bs-target="#home-tab-pane" type="button" role="tab" aria-controls="home-tab-pane" aria-selected="true" onClick={()=>{
      fetchuploaded_docs()
    }}>Uploaded Documents ({uploadedDocuments.length})</button>
  </li>
  <li class="nav-item" role="presentation">
    <button class={`nav-link bg-white me-lg-4 ${value==="followed_documents" ? 'text-primary active' : 'text-secondary'}`} id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="profile-tab-pane" aria-selected="false" onClick={()=>{
      fetchfollowed_docs()
    }}>Followed Documents ({favouritedocs.length})</button>
  </li>
  <li class={`nav-item ${user_id===user.user_id ? '':'d-none'}`} role="presentation">
    <button onClick={()=>{
      fetchcreated_flashsets()
    }} class={`nav-link bg-white me-lg-4 ${value==="uploaded_flashcards" ? 'text-primary active' : 'text-secondary'}`} id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false">Created Flashsets ({createdFlashsets.length})</button>
  </li>
  <li class={`nav-item ${user_id===user.user_id ? '':'d-none'}`} role="presentation">
    <button onClick={()=>{
      fetchsaved_flashsets()
    }} class={`nav-link bg-white ${value==="followed_flashcards" ? 'text-primary active' : 'text-secondary'}`} id="disabled-tab" data-bs-toggle="tab" data-bs-target="#disabled-tab-pane" type="button" role="tab" aria-controls="disabled-tab-pane" aria-selected="false">Saved Flashsets ({savedFlashsets.length})</button>
  </li>
</ul>
          </div>
...
{uploadedDocuments && (
  <div class={`tab-pane fade bg-light mt-3 ${value==="uploaded_documents" ? 'show active' : ''}`} id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab" tabindex="0">
...
<button data-bs-toggle="modal" data-bs-target="#studylist_modal" className={`bg-transparent border-0 ${x.study_list_status ? 'd-none':''}`} style={{color:'#8587EA'}} onClick={()=>{
  ...
  <path d="M7.50016 11.2261L3.81425 7.54021L4.55144 6.79102L6.97933 9.21891V0.208496H8.021V9.21891L10.4489 6.79102L11.1861 7.54021L7.50016 11.2261ZM1.89118 14.7918C1.41175 14.7918 1.01145 14.6312 0.690267 14.3101C0.369086 13.9889 0.208496 13.5886 0.208496 13.1091V10.5851H1.25016V13.1091C1.25016 13.2694 1.31693 13.4163 1.45048 13.5498C1.58403 13.6834 1.73094 13.7502 1.89118 13.7502H13.1091C13.2694 13.7502 13.4163 13.6834 13.5498 13.5498C13.6834 13.4163 13.7502 13.2694 13.7502 13.1091V10.5851H14.7918V13.1091C14.7918 13.5886 14.6312 13.9889 14.3101 14.3101C13.9889 14.6312 13.5886 14.7918 13.1091 14.7918H1.89118Z" fill="#8E9696"/>
                        ...
                        <span  style={{fontSize:'14px',color:'#AAB0B0'}}  className='ms-3 d-flex align-items-center'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chat-left" viewBox="0 0 16 16">
...
<div className="m-0 d-flex align-items-center mt-2">
        ...
        <path d="M15.1891 18.8335H4.73237V7.37514L11.2628 0.924805L11.7236 1.38556C11.8184 1.48037 11.8982 1.6019 11.9629 1.75014C12.0277 1.89837 12.0601 2.03459 12.0601 2.15879V2.32306L10.9944 7.37514H18.234C18.6693 7.37514 19.0586 7.54675 19.4018 7.88996C19.7451 8.23317 19.9167 8.62246 19.9167 9.05783V10.3399C19.9167 10.4347 19.906 10.5382 19.8846 10.6504C19.8633 10.7626 19.8352 10.8661 19.8005 10.9609L16.9559 17.7037C16.813 18.0242 16.5727 18.2926 16.2348 18.509C15.8969 18.7253 15.5483 18.8335 15.1891 18.8335ZM5.77404 17.7918H15.1891C15.336 17.7918 15.4862 17.7518 15.6398 17.6716C15.7934 17.5915 15.9103 17.4579 15.9904 17.271L18.875 10.5001V9.05783C18.875 8.87086 18.8149 8.71729 18.6947 8.5971C18.5745 8.4769 18.4209 8.41681 18.234 8.41681H9.70031L10.9062 2.7277L5.77404 7.81986V17.7918ZM4.73237 7.37514V8.41681H1.16667V17.7918H4.73237V18.8335H0.125V7.37514H4.73237Z" fill="#8E9696"/>
                        ...
                        <span data-bs-toggle="tooltip" data-bs-placement="bottom"
        data-bs-custom-class="custom-tooltip"
        data-bs-title="Comments" style={{fontSize:'14px',color:'#AAB0B0'}} className='ms-3 d-flex align-items-center'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chat-left" viewBox="0 0 16 16">
        <path d="M14 1a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H4.414A2 2 0 0 0 3 11.586l-2 2V2a1 1 0 0 1 1-1zM2 0a2 2 0 0 0-2 2v12.793a.5.5 0 0 0 .854.353l2.853-2.853A1 1 0 0 1 4.414 12H14a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z"/>
...
<div class={`tab-pane fade bg-light mt-3 ${value==="followed_documents" ? 'show active' : ''}`} id="profile-tab-pane" role="tabpanel" aria-labelledby="profile-tab" tabindex="0">
...
{/* --------------------------------------------Created Flashcards Layout Section--------------------------------------- */}
  ...
  <div class={`tab-pane fade bg-light mt-3 ${value==="uploaded_flashcards" ? 'show active' : ''}`} id="contact-tab-pane" role="tabpanel" aria-labelledby="contact-tab" tabindex="0">
 <div>
 <div className='d-flex justify-content-between bg-white shadow-sm py-3 px-3 align-items-center mb-3'>
...
{/* --------------------------------------------Saved flashsets layout section------------------------------------------- */}
  ...
  <div class={`tab-pane fade bg-light mt-3 ${value==="followed_flashcards" ? 'show active' : ''}`} id="disabled-tab-pane" role="tabpanel" aria-labelledby="disabled-tab" tabindex="0">
<div>
<div className='d-flex justify-content-between bg-white shadow-sm py-3 px-3 align-items-center mb-3'>
