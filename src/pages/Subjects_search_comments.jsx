...
const Subject_search_comments = ({course_id,course_name,setreportdropdownstate,searchvalue,setCount}) => {
...
const[count1,setCount1]=useState(0)
 ...
 const[replies_layout,setreplies_layout]=useState(false)
  ...
  useEffect(()=>{
     ...
     axios.get(`${ipaddress}/UserUpdateDetails/${userdata.user_id}/`)
     .then((r)=>{
...
const searchcomments = () => {
  ...
  axios.get(`${ipaddress}/searchcoursecomments/${course_id}/${searchvalue}/${user.user_id}/`)
    .then((response) => {
      console.log("Searched Comment successful", response.data);
...
const removeImage4 = (index) => {
  setRepliesImage4((prevImages) => prevImages.filter((_, i) => i !== index));
};
...
const removereply_image4 = (index) => {
  setReplies_reply_image4((prevImages) => prevImages.filter((_, i) => i !== index));
};
...
function handleLike1(discussion_id) {
 ...
 const editPosts=(discid)=>{
   ...
   const foundDiscussion = searchedComments.find((x) => discid === x.discid);
...
axios.post(`${ipaddress}/coursediscussionpinningview/${user.user_id}/${course_id}/`,formdata)
.then((r)=>{
...
setindex1(-1)
    ...
    const toastLiveExample = document.getElementById('liveToast')
    document.getElementById('toastbody').style.color="green"
    document.getElementById('toastbody').textContent="Comment Pinned Successfully !!!"
const toastBootstrap = bootstrap.Toast.getOrCreateInstance(toastLiveExample)
toastBootstrap.show()
setindex1(-1)
  }
...
axios.delete(`${ipaddress}/Unpiningcoursecomments/${user.user_id}/${course_id}/${discid}/`)
.then((r)=>{
  console.log("UnPinned Successfully",r.data)
    ...
    const toastLiveExample = document.getElementById('liveToast')
    document.getElementById('toastbody').style.color="green"
    document.getElementById('toastbody').textContent="Comment Unpinned Successfully !!!"
const toastBootstrap = bootstrap.Toast.getOrCreateInstance(toastLiveExample)
toastBootstrap.show()
})
.catch(()=>{
  ...
  return (
    <div className={`${searchvalue.length>0 ? '':'d-none'}`}>
      <h6 className={`text-center py-2 ${searchedComments.length>0 ? 'd-none':''}`} style={{color:'#5d5fe3'}}>No Matched comments available !!!</h6>
  <h6 className={`pt-2 ${searchedComments.length>0 ? '':'d-none'}`} style={{color:'#5d5fe3'}}>Searched Comments</h6>
  ...
  <p className={x.user_details.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{x.user_details.nickname.slice(0,1)}</span><span>{x.user_details.nickname.slice(-1)}</span></p>
          ...
          <div className={`col-3 d-flex justify-content-between align-items-center p-0 ${x.pinned_status ? 'col-md-3':'col-md-2'}`}>
            <span onClick={()=>{
              unpin(x.discid)
            }} style={{cursor:'pointer'}} className={`border p-1 px-2 rounded ${x.pinned_status ? '':'d-none'}`}><i class="fa-solid fa-link-slash d-inline d-md-none"></i> <span className='d-none d-md-inline'>Followed</span></span>
...
<path d="M17.4998 27.0354C17.0988 27.0354 16.7555 26.8926 16.4699 26.607C16.1843 26.3214 16.0415 25.9781 16.0415 25.5771C16.0415 25.176 16.1843 24.8327 16.4699 24.5471C16.7555 24.2615 17.0988 24.1187 17.4998 24.1187C17.9009 24.1187 18.2442 24.2615 18.5298 24.5471C18.8154 24.8327 18.9582 25.176 18.9582 25.5771C18.9582 25.9781 18.8154 26.3214 18.5298 26.607C18.2442 26.8926 17.9009 27.0354 17.4998 27.0354ZM17.4998 18.9585C17.0988 18.9585 16.7555 18.8157 16.4699 18.5301C16.1843 18.2445 16.0415 17.9012 16.0415 17.5001C16.0415 17.0991 16.1843 16.7558 16.4699 16.4702C16.7555 16.1846 17.0988 16.0418 17.4998 16.0418C17.9009 16.0418 18.2442 16.1846 18.5298 16.4702C18.8154 16.7558 18.9582 17.0991 18.9582 17.5001C18.9582 17.9012 18.8154 18.2445 18.5298 18.5301C18.2442 18.8157 17.9009 18.9585 17.4998 18.9585ZM17.4998 10.8815C17.0988 10.8815 16.7555 10.7388 16.4699 10.4532C16.1843 10.1676 16.0415 9.82424 16.0415 9.42318C16.0415 9.02214 16.1843 8.67882 16.4699 8.39323C16.7555 8.10764 17.0988 7.96484 17.4998 7.96484C17.9009 7.96484 18.2442 8.10764 18.5298 8.39323C18.8154 8.67882 18.9582 9.02214 18.9582 9.42318C18.9582 9.82424 18.8154 10.1676 18.5298 10.4532C18.2442 10.7388 17.9009 10.8815 17.4998 10.8815Z" fill="#2A3941"/>
  ...
  <button className={x.user_details.nickname !=user.nickname || x.created_at.includes("day") || x.created_at.includes("week") || x.created_at.includes("year") ? 'd-none' : 'd-flex align-items-center bg-transparent border-0 my-2'} data-bs-toggle="modal" data-bs-target="#editModal" onClick={()=>{
              editPosts(x.discid)
          ...
          }} className={`bg-transparent border-0 my-2 ${x.report_status ? 'd-none':'d-flex align-items-center'}`} style={{height:'20px'}}><span className='dropdownmenu'><svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 35 35" fill="none">
...
}}>{translate_value.dashboard.view_all} {x.reply_count} {translate_value.dashboard.replies}</button> : <p></p>
}
</div>
...
{y.images_attached.map((a)=>{
  ...
  <span data-bs-toggle="modal" data-bs-target="#replyforreply_modal4" onClick={()=>{
        ...
        {repliesImage4.length > 0 &&
          repliesImage4.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              ...
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removeImage4(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle" viewBox="0 0 16 16">
...
<div class="modal fade" id="replyforreply_modal4" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
      ...
      <div class="modal-content">
        ...
        {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
          ...
          replies_reply_Image4.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              <img src={URL.createObjectURL(image)} width={50} alt={`Selected Image ${index + 1}`} />
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removereply_image4(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
  <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
</svg></button>
            </div>
...
export default Subject_search_comments