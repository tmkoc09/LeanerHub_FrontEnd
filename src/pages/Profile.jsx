...
const[count,setCount]=useState(0)
  ...
  useEffect(()=>{
    axios.get(`${ipaddress}/docAndLikesCount/${userData.user_id}/`)
    .then((r)=>{
    ...
    axios.get(`${ipaddress}/UserStudyInfo/${user_id}/`)
    .then((r)=>{
    ...
    axios.get(`${ipaddress}/UserUpdateDetails/${user_id}/`)
  .then((r)=>{
  ...
  axios.get(`${ipaddress}/CoursesView/${userData.user_id}/`)
    .then((r) => {
  ...
  const fetchAllUniversities=()=>{
    axios.get(`${ipaddress}/universities/`)
    .then((r)=>{
      setUniversitydata(r.data)
      // console.log("University data : ",r.data)
    })
    axios.get(`${ipaddress}/Semesters/`)
    .then((p)=>{
  ...
  const handleSubmit=(event)=>{
    ...
    axios.post(`${ipaddress}/UserStudyInfo/${user_id}/`,formData)
    .then((r)=>{
...
const adduniversityname=(universityName,event,universityId)=>{
  ...
  axios.get(`${ipaddress}/Programs/${universityId}`)
  .then((r)=>{
...
const deleteAccount=()=>{
  axios.delete(`${ipaddress}/DeleteAccount/${user_id}`)
  .then((r)=>{
...
setTimeout(()=>{
  axios.post(`${ipaddress}/User_Logout/${user_id}/`)
    .then((r)=>{
      sessionStorage.removeItem("user")
...
formdata.append('attached_image',image)
  ...
  axios.post(`${ipaddress}/ReportUserView/${userData.user_id}/${user_id}/${report}/`,formdata)
  .then((r)=>{
    ...
    {loading ? (<Preloader/>):(
       ...
       <Mainsidebar count={count}></Mainsidebar>
         ...
         <Navbar count={count}></Navbar>
           ...
           {/* <div className={`text-end px-3 mt-2 ${user_id === userData.user_id ? '':'d-none'}`}>
             <Link data-bs-toggle="modal" data-bs-target="#add_details" type="button" class="btn btn-primary btn-sm" id='add-details-btn'  onClick={fetchAllUniversities}>
             <i class="fa-solid fa-plus me-2"></i>Add Details
 </Link>
 <button type="button" class="btn btn-primary btn-sm ms-3" id='add-details-btn' data-bs-toggle="modal" data-bs-target="#set_bed_time" onClick={fetchAllUniversities}>
 <i class="fa-regular fa-clock me-2"></i>Set Bed Time
 </button>
             </div> */}
           {/* <div className={joinedUniversity.length<=0 ? 'd-block mt-5' : 'd-none'}><h3 className='text-center'>ADD YOUR UNIVERSITY DETAILS !!!</h3></div> */}
 ...
 <div className={`d-flex justify-content-end ${user_id===userData.user_id ? '':''}`}>
  ...
  <path d="M21.4648 25.1669H6.82532V9.1252L15.9679 0.0947266L16.613 0.739784C16.7457 0.872516 16.8574 1.04266 16.9481 1.2502C17.0388 1.45772 17.0841 1.64842 17.0841 1.82231V2.05228L15.5922 9.1252H25.7276C26.3371 9.1252 26.8821 9.36545 27.3626 9.84595C27.8431 10.3264 28.0833 10.8714 28.0833 11.481V13.2758C28.0833 13.4086 28.0684 13.5535 28.0385 13.7105C28.0086 13.8676 27.9693 14.0125 27.9207 14.1452L23.9383 23.5852C23.7383 24.0339 23.4017 24.4097 22.9287 24.7126C22.4557 25.0154 21.9677 25.1669 21.4648 25.1669ZM8.28365 23.7085H21.4648C21.6704 23.7085 21.8807 23.6525 22.0957 23.5403C22.3108 23.4281 22.4744 23.2411 22.5865 22.9794L26.625 13.5002V11.481C26.625 11.2192 26.5409 11.0042 26.3726 10.8359C26.2043 10.6677 25.9893 10.5835 25.7276 10.5835H13.7804L15.4688 2.61877L8.28365 9.7478V23.7085ZM6.82532 9.1252V10.5835H1.83333V23.7085H6.82532V25.1669H0.375V9.1252H6.82532Z" fill="#FF845D"/>
   ...
   <div className="col-lg-3 col-sx-6 d-flex align-items-center border-end justify-content-center" style={{letterSpacing:'2px',height:'40px'}}>
     <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="#FF845D" class="bi bi-file-earmark-arrow-up" viewBox="0 0 16 16">
             ...
             return(
                 <div className="row">
                  <div className="col-12">
                   <span className=''><i class="fa-solid fa-building-columns  me-2"></i>{x.university_id.university_name}</span>
           </div>
           <div className="col-12 mt-3"><span className=''>
             {/* <i class="fa-solid fa-graduation-cap  me-2"></i> */}
             <i class="fa-solid fa-envelope me-2"></i>{userdetails.email}</span></div>
               </div>
             )
           })}
 ...
 {/* <div className={joinedUniversity.length>0 ? 'd-block container bg-white shadow-md d-flex justify-content-between py-3 border mt-4 mx-auto' : 'd-none'}>
 <div className=''>
 <Link to='/flashset' className='text-decoration-none text-dark'><img src={require('../img/flash-cards-removebg-preview.png')} width={30} className='me-2 me-sm-3' alt="" />My Flashcards</Link>
 </div>
 ...
 <div
       ...
       >
         ...
         <button data-bs-dismiss="modal"  onClick={()=>{
                 setUniversityNamesVisible(false)
               }} className='bg-transparent border-0'><i class="fa-solid fa-circle-xmark fs-5"></i></button>
 ...
 <div className='px-2 m-0 bg-info-subtle' style={{listStyleType:'none'}}>
  ...
  searchResults.map((university,index) => (
   <li key={index}><a href='' className='text-decoration-none text-dark ' style={{fontSize:'14px'}} onClick={()=>{
 ...
 <ul className='px-2 m-0' style={{listStyleType:'none'}}>
   {universitydata.map((x, index) => {
     ...
     return (
       !isMatch && (
         <li key={index}>
           <a href='' className='text-decoration-none text-dark ' style={{fontSize:'14px'}} onClick={()=>{
     adduniversityname(x.university_name,event,x.university_id)
                 ...
                 <div className="row mt-3" id='programs'>
                   <div className="col-12">
                     <select
                       name="program"
                       class="form-select form-select-md px-2 py-3 mb-3 shadow-none"
               ...
               {/* <div className="p-2 px-4 text-center mt-2 mb-2">
 <a href='#' className='google-signup text-decoration-none'><img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c1/Google_%22G%22_logo.svg/2048px-Google_%22G%22_logo.svg.png" width='30px' alt="" className='me-2'/>
 ...
 {/* EDIT USER DETAILS MODAL FORM */}
       ...
       >
         <div class="modal-dialog">
           <div class="modal-content">
             <div class="d-flex p-2 justify-content-between align-items-center px-1 px-md-3">
             <h6
                 class=" fw-bold text-center m-0" style={{color:'#5d5fe3'}}
                 id="staticBackdropLabel"
               >
                 Edit Your Details
               </h6>
               <button
                 type="button"
                 class="ms-auto btn btn-sm border-0 d-flex align-items-center"
             ...
             <div class="modal-body">
               <form action="" encType="multipart/form-data" onSubmit={sendEditedDetails}  className="p-2 px-4">
                 <div className="row">
                   ...
                   <input type="text" className='form-control' maxLength={6} value={editedNickname} onChange={(e)=>{
                   ...
                   <label htmlFor="file" className='bg-info text-white py-1 px-4 rounded-pill '><i class="fa-solid fa-cloud-arrow-up me-2"></i>Upload Profile Image</label>
       ...
       >
         <div class="modal-dialog">
           <div class="modal-content">
             <div className='d-flex justify-content-between px-3 pt-3'>
               <h6
                 class=" fw-medium text-center text-primary"
                 id="staticBackdropLabel"
               >
                 Report the User
               </h6>
