...
//     const handleTermImage = async (event) => {
//         setTermImage(event.target.files);
//     };

// const [definitionImage, setDefinitionImage] = useState([]);
// .8000
//     const handleDefinitionImage = async (event) => {
//         setDefinitionImage(event.target.files);
//     };

// const postQuestion=async (e)=>{
//   const user=JSON.parse(sessionStorage.getItem('user'))
//   const formData = new FormData();
//         for (const file of termImage) {
...
//         }
//         try {
//           const response = await fetch(`127.0.0.1.8000/${user.user_id}/${id}/${discussionquestion}/`, {
//               method: 'POST',
//               body: formData,
//           });
...
axios.post(`${ipaddress2}/flashcardsets/create/${user.user_id}/${courseid}/${semesterid}/`,formdata)
.then((r)=>{
...
//     newFlashcards[index][name] = files[0];
//   } else {
//     // Assign null to image fields when no files are uploaded
//     if (name === "t_image" || name === "d_image") {
//       newFlashcards[index][name] = null;
//     } else {
//       newFlashcards[index][name] = value;
//     }
//   }
...
const handleSubmit = async (event) => {
  ...
  try {
    ...
    const response = await axios.post(`${ipaddress2}/flashcards/create/${flashsetId.flashset_id}/`, formData, {
      headers: {
    ...
    <div onClick={()=>{
        ...
        <Navbar count={count}></Navbar>
                ...
                <select style={{height:'50px'}}
                      name="courses"
                      class="form-select px-5 ps-4 bg-light shadow-none"
                ...
                <select
                      name="semester"
                      class="form-select form-select-md px-5 ps-4 shadow-none bg-light" style={{height:'50px'}}
                      aria-label="Large select example"
                      onChange={semesterId}
                      value={semesterid}
                    >
...
{flashcard.d_image && (
       ...
       <div className='text-end mt-3'>
          <button className='btn btn-sm  text-white fw-medium' style={{backgroundColor:'#5D5FE3',fontSize:'14px',cursor:'pointer'}} type="button" onClick={addFlashcard}><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-plus-square" viewBox="0 0 16 16">
