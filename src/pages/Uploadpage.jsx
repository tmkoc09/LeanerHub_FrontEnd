...
const handleNextPage = () => {
  ...
  const chapternameData=(e)=>{
    ...
    axios.get(`${ipaddress}/ChapterSuggestions/${e.target.value}/`)
    .then((p)=>{
...
document.getElementById('details').style.display='block'
  ...
  useEffect(()=>{
    ...
    axios.get(`${ipaddress}/category_view/`)
    .then((p)=>{
    ...
    axios.get(`${ipaddress}/subCategoryView/`)
    .then((p)=>{
...
<path d="M29.125 43.5H30.875V31.375L36.5 37L37.75 35.75L30 28L22.25 35.75L23.5 37L29.125 31.375V43.5ZM17 51.75C15.9167 51.75 15.0208 51.3958 14.3125 50.6875C13.6042 49.9792 13.25 49.0833 13.25 48V12C13.25 10.9167 13.6042 10.0208 14.3125 9.3125C15.0208 8.60417 15.9167 8.25 17 8.25H36.625L46.75 18.375V48C46.75 49.0833 46.3958 49.9792 45.6875 50.6875C44.9792 51.3958 44.0833 51.75 43 51.75H17ZM35.75 19.25V10H17C16.5 10 16.0417 10.2083 15.625 10.625C15.2083 11.0417 15 11.5 15 12V48C15 48.5 15.2083 48.9583 15.625 49.375C16.0417 49.7917 16.5 50 17 50H43C43.5 50 43.9583 49.7917 44.375 49.375C44.7917 48.9583 45 48.5 45 48V19.25H35.75Z" fill="#5D5FE3"/>
                ...
                <label
                  for="fileInput"
                  class="custom-file-input rounded border-0 text-white px-5 py-2 fw-normal" style={{backgroundColor:'#5D5FE3'}}
                >
                Browse
                </label>
                {/* {file && (
...
<div class={`mb-4 col-md-12`}>
  <label htmlFor="" className='mb-2' style={{color:'#8E9696'}}>Document Title <span style={{color:'red'}}>*</span></label>
              <input type="text" className='form-control py-3' value={document_title} onChange={(e)=>{
                ...
                <div class={`mb-4 col-md-6 ${course_id>0 ? '':'d-none'}`}>
  <label htmlFor="" className='mb-2' style={{color:'#8E9696'}}>Document Category <span style={{color:'red'}}>*</span></label>
...
<div class={`mb-4 col-md-6 ${course_id>0 ? 'd-none':''}`}>
  <label htmlFor="" className='mb-2' style={{color:'#8E9696'}}>Document Category <span style={{color:'red'}}>*</span></label>
                <select
                      name="courses"
                      class="form-select form-select-md px-5 ps-4 py-3 shadow-none"
...
<div className={`col-12 ${chaptervisibility ? '':'d-none'}`}>
  ...
  <input type="text" value={Chaptername} onChange={chapternameData} className='form-control py-3' name="" id="" />
  <div className={`bg-white ps-3 pt-2 border border-top-0 ${chapternames.length>0 && chapterlayoutstatus ? '':'d-none'}`}>
    {chapternames.map((x)=>{
      return(
        <>
        <p className='m-0 mb-1' style={{cursor:'pointer',fontSize:'14px'}} onClick={()=>{
          setChaptername(x.chapter_name)
          setchapterlayoutstatus(false)
        }}>{x.chapter_name}</p> 
        </>
      )
    })}
  ...
  <div class="mb-4 col-md-6">
  <label htmlFor="" className='mb-2' style={{color:'#8E9696'}}>Sub Sub Category</label>
                <select
                      name="courses"
                      class="form-select form-select-md px-5 ps-4 py-3 shadow-none"
                      aria-label="Large select example"
                    ...
                    >
                      <option value="" selected>Select the Sub Sub Category</option>
  ...
  <div class="mb-4 col-md-6">
  <label htmlFor="" className='mb-2' style={{color:'#8E9696'}}>Sub sub sub category</label>
  ...
  <label htmlFor="" className='mb-2' style={{color:'#8E9696'}}>Sub Sub Sub Category</label>
                <select
                    ...
                    >
                      <option value="" selected>Select the Sub Sub Sub Category</option>
...
<div className='mt-4 text-center'>
  <button disabled={!preview_btn_status1} className='btn text-white fw-medium' style={{backgroundColor:'#5D5FE3'}} onClick={()=>{
    document.getElementById('details').style.display='none'
