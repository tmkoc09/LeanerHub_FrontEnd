...
const University_discussion_offcanvases = ({count,usercomments_status,setCount}) => {
  ...
  const[replies,setReplies]=useState("")
    ...
    useEffect(()=>{
      ...
      fetchusercomments()
    },[usercomments_status,count1,count])


const fetchusercomments=()=>{
  axios.get(`${ipaddress}/UserComments/${user.university_id}/${user.user_id}/`)
  .then((r)=>{
      console.log("User discussion for university",r.data)
...
const [repliesImage5, setRepliesImage5] = useState([]);
const clearInput = (index) => {
  const inputElement = document.getElementsByClassName('reply-input5');
  if (inputElement) {
    inputElement[index].value = '';
  }
};

const handleReplyImage5 = (event) => {
  const files=event.target.files
     // Ensure that 'files' is not null or undefined
  if (files && files.length > 0) {
    setRepliesImage5(Array.from(files));
  } else {
    setRepliesImage5([]);
  }
};

const removeImage5 = (index) => {
  setRepliesImage5((prevImages) => prevImages.filter((_, i) => i !== index));
};

const postReplies= async (dis_id,index)=>{
  const user=JSON.parse(sessionStorage.getItem('user'))
  const formData = new FormData();
        for (const file of repliesImage5) {
...
const [replies_reply_Image5, setReplies_reply_image5] = useState([]);
...
const handleReply_reply_Image5 = (event) => {
  const files=event.target.files
     // Ensure that 'files' is not null or undefined
  if (files && files.length > 0) {
    setReplies_reply_image5(Array.from(files));
  } else {
    setReplies_reply_image5([]);
  }
};

const removereply_Image5 = (index) => {
  setReplies_reply_image5((prevImages) => prevImages.filter((_, i) => i !== index));
};

const postreply_for_replies= async ()=>{
  const user=JSON.parse(sessionStorage.getItem('user'))
  const formData = new FormData();
        for (const file of replies_reply_Image5) {
            formData.append('images_attached', file);
        }
        try {
...
const pincomment=(discId)=>{
  axios.post(`${ipaddress}/UniversityPinnedCommentsView/${user.user_id}/${discId}/${user.university_id}/`)
  .then((r)=>{
...
const unpin=(discid)=>{
  ...
  return (
    <div>
        {/* PARTICULAR USER COMMENT SECTION */}
        <div class="modal fade" id="usercommentsmodal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title" id="exampleModalLabel">Welcome <span style={{color:'#5d5fe3'}}>{user.nickname}</span>, View your discussions here</h6>
        <button type="button" className='btn p-0' data-bs-dismiss="modal"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293z"/>
</svg></button>
      </div>
      <div class="modal-body" style={{height:'90vh',overflowY:'scroll'}}>
      <h6 className={`text-secondary text-center mt-3 ${userComments.length>0 ? 'd-none':''}`}>Post Your Comments !!!</h6>
      ...
      <p className={x.user_id.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{x.user_id.nickname.slice(0,1)}</span><span>{x.user_id.nickname.slice(-1)}</span></p>
              ...
              <div className={`col-2 d-flex justify-content-between align-items-center p-0 ${x.pinned_status ? 'col-md-3':'col-md-2'}`} style={{position:'relative'}}>
  <button onClick={()=>{
    unpin(x.id)
...
<path d="M17.5 27.0354C17.099 27.0354 16.7557 26.8926 16.4701 26.607C16.1845 26.3214 16.0417 25.9781 16.0417 25.5771C16.0417 25.176 16.1845 24.8327 16.4701 24.5471C16.7557 24.2615 17.099 24.1187 17.5 24.1187C17.9011 24.1187 18.2444 24.2615 18.53 24.5471C18.8156 24.8327 18.9584 25.176 18.9584 25.5771C18.9584 25.9781 18.8156 26.3214 18.53 26.607C18.2444 26.8926 17.9011 27.0354 17.5 27.0354ZM17.5 18.9585C17.099 18.9585 16.7557 18.8157 16.4701 18.5301C16.1845 18.2445 16.0417 17.9012 16.0417 17.5001C16.0417 17.0991 16.1845 16.7558 16.4701 16.4702C16.7557 16.1846 17.099 16.0418 17.5 16.0418C17.9011 16.0418 18.2444 16.1846 18.53 16.4702C18.8156 16.7558 18.9584 17.0991 18.9584 17.5001C18.9584 17.9012 18.8156 18.2445 18.53 18.5301C18.2444 18.8157 17.9011 18.9585 17.5 18.9585ZM17.5 10.8815C17.099 10.8815 16.7557 10.7388 16.4701 10.4532C16.1845 10.1676 16.0417 9.82424 16.0417 9.42318C16.0417 9.02214 16.1845 8.67882 16.4701 8.39323C16.7557 8.10764 17.099 7.96484 17.5 7.96484C17.9011 7.96484 18.2444 8.10764 18.53 8.39323C18.8156 8.67882 18.9584 9.02214 18.9584 9.42318C18.9584 9.82424 18.8156 10.1676 18.53 10.4532C18.2444 10.7388 17.9011 10.8815 17.5 10.8815Z" fill="#2A3941"/>
    ...
    {/* <button className={x.created_at.includes("day") || x.created_at.includes("week") || x.created_at.includes("year")
  && user.user_id===x.user_id.user_id  ? 'd-none' : 'd-block bg-transparent border-0 my-2'} data-bs-toggle="modal" data-bs-target="#editModal" onClick={()=>{
                editPosts(x.id)
              }} style={{height:'20px'}}> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="15" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
              <path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001m-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708z"/>
            </svg><span className="ms-2">{translate_value.common_words.edit}</span></button> */}
...
getreplies_for_reply(y.id,index2)
        ...
        {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
              <div class="input-group border rounded pe-3 bg-light">
                    <input key={index}
                      ...
                      className="form-control py-3 ps-3 shadow-none border-0 bg-light reply-input5" placeholder={translate_value.dashboard.reply_here}
                      style={{position:'relative'}}/>
...
<div className='d-flex gap-3 mt-3'>
        {repliesImage5.length > 0 &&
          repliesImage5.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              ...
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removeImage5(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle" viewBox="0 0 16 16">
      ...
      <div class="modal-content">
        ...
        {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
              <div class="input-group bg-light border rounded pe-3">
                    ...
                    accept="image/*"
                    multiple
                    onChange={handleReply_reply_Image5}
                    className="bg-light text-center p-3 btn"
                  />
                  <label
                    htmlFor="fileInput5"
                    ...
                    <button onClick={()=>{
                      postreply_for_replies()
                    }} className='text-secondary h-100 bg-transparent border-0 ms-2 outline-0' ><span aria-hidden="true"></span><i class="fa-solid fa-paper-plane"></i></button>
                    </div>
                  </div>
                 
            </div>
            <div className='d-flex gap-3 mt-3'>
        {replies_reply_Image5.length > 0 &&
          replies_reply_Image5.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              <img src={URL.createObjectURL(image)} width={50} alt={`Selected Image ${index + 1}`} />
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removereply_Image5(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
  <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
</svg></button>
            </div>
...
export default University_discussion_offcanvases