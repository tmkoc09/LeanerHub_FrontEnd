...
const Showpdfpage = ({settranslated_pdf_url}) => {
  ...
  useEffect(()=>{
    ...
    axios.get(`${ipaddress}/SpecificDocumentDisplay/${userdata.user_id}/${id}/`)
    .then((response) => {
    ...
    axios.get(`${ipaddress}/DocumentFollowResponse/${id}/`)
    .then((r)=>{
...
// ------------------------------------------------------TRANSLATED PDF----------------------------------------------------
  ...
  const translatePdf=()=>{
    axios.get(`${ipaddress}/TranslatePDFAPIView/${lang}/${id}`)
    .then((r)=>{
...
const postQuestion=async (e)=>{
  ...
  const formData = new FormData();
        ...
        // }
        try {
          const response = await fetch(`${ipaddress}/DocumentsComments/${user.user_id}/${id}/${discussionquestion}/`, {
              method: 'POST',
              body: formData
          });
...
const postReplies=async(dis_id,index)=>{
  ...
  if(replies.length>0){
    try {
      const response = await fetch(`${ipaddress}/DocCommentsRepliesNewView/${user.user_id}/${dis_id}/${replies}/`, {
          method: 'POST',
          body: formData,
      });
...
const postreply_for_replies= async ()=>{
  ...
  const formData = new FormData();
        ...
        try {
          const response = await fetch(`${ipaddress}/adocumentpostrepliesreplies/${user.user_id}/${reply_id}/${reply_for_reply}/`, {
              method: 'POST',
              body: formData,
          });
...
const searchcomments = (value) => {
  ...
  axios.get(`${ipaddress}/searchdocumentcomment/${id}/${value}/${user.user_id}/`)
    .then((response) => {
...
const getdiscussion=()=>{
  const user=JSON.parse(sessionStorage.getItem('user'))
  axios.get(`${ipaddress}/DCRV/${user.user_id}/${id}/`)
  .then((r)=>{
...
function handleLike1(discussion_id) {
 axios.delete(`${ipaddress}/DocumentCommentsLikes/${user.user_id}/${discussion_id}/`)
 .then((r)=>{
...
function handleLike(discussion_id) {
  axios.post(`${ipaddress}/DocumentCommentsLikes/${user.user_id}/${discussion_id}/`)
  .then((r)=>{
...
function handledislike(discussion_id) {
  axios.post(`${ipaddress}/doc_comments_dis_likes_view/${user.user_id}/${discussion_id}/`)
  .then((r)=>{
...
function handleReplyLike1(discussion_reply_id,discid,index) {
  axios.delete(`${ipaddress}/CommentRepliesLikes/${userdata.user_id}/${discussion_reply_id}/`)
  .then((r)=>{
 ...
 function handleReplyLike(discussion_reply_id,discid,index) {
   axios.post(`${ipaddress}/CommentRepliesLikes/${userdata.user_id}/${discussion_reply_id}/`)
   .then((r)=>{
...
function handleReplies_reply_like(replies_reply_id,disc_replyid) {
  axios.post(`${ipaddress}/adocumentpostrepliesreplieslikesanddislikes/${user.user_id}/${replies_reply_id}/`)
  .then((r)=>{
...
function handlereplies_replydislike(replies_reply_id,disc_replyid) {
  axios.patch(`${ipaddress}/adocumentpostrepliesreplieslikesanddislikes/${user.user_id}/${replies_reply_id}/`)
  .then((r)=>{
...
function handlereplydislike(reply_id,discussion_id,index) {
  axios.post(`${ipaddress}/DocumentReplyDisLikeView/${user.user_id}/${reply_id}/`)
  .then((r)=>{
...
// ----------------------------------------------- RATINGS FOR DOCUMENT-------------------------------------------------------
  ...
  const sendRatingToBackend = async (rating) => {
      await axios.post(`${ipaddress}/DocumentRating/${userdata.user_id}/${id}/`,rating)
      .then((r)=>{
...
const followDocument=()=>{
    axios.post(`${ipaddress}/FavouriteDocs/${userdata.user_id}/${id}/`)
    .then((r)=>{
...
const unfollowDocument=()=>{
  axios.delete(`${ipaddress}/FavouriteDocs/${userdata.user_id}/${id}/`)
  .then((r)=>{
...
const dislikeDocument=()=>{
  axios.post(`${ipaddress}/DisLikeDocuments/${userdata.user_id}/${id}/`)
  .then((r)=>{
...
const deletePost=(discussion_id)=>{
  axios.post(`${ipaddress}/documentcommentdelete/${discussion_id}/`)
  .then((r)=>{
...
const deleteReply=(disc_reply_id,discid,index)=>{
  axios.delete(`${ipaddress}/documentcommentreplydelete/${disc_reply_id}/`)
  .then((r)=>{
...
const deleteReply_for_reply=(reply_reply_id,disc_reply_id,discid)=>{
  axios.delete(`${ipaddress}/adocumentpostrepliesreplies/${user.user_id}/${reply_reply_id}/`)
  .then((r)=>{
...
const getreplies=(discid,index)=>{
  axios.get(`${ipaddress}/adocumentcommentsreplies/${discid}/${user.user_id}/`)
  .then((r)=>{
...
const getreplies_for_reply=(particular_reply_id)=>{
  axios.get(`${ipaddress}/adocumentpostrepliesreplies/${user.user_id}/${particular_reply_id}/`)
  .then((r)=>{
...
const sendreport=()=>{
  ...
  axios.post(`${ipaddress}/document_report/${id}/`,formdata)
  .then((r)=>{
...
const addtostudylist=()=>{
  axios.post(`${ipaddress}/UserStudyListView/${userdata.user_id}/${id}/`)
  .then((r)=>{
    // console.log(r.data)
...
const extractTextFromPdf = async () => {
  try {
    const response = await axios.get(pdfurldata.documnet_id.document_url, {
...
const sendEditedData =() => {
  ...
  axios.put(`${ipaddress}/DocumentsComments/${user.user_id}/${id}/${discussionId}/`,formData)
 .then((r)=>{
...
axios.post(`${ipaddress}/documentdiscussionpinningview/${user.user_id}/${id}/`,formdata)
.then((r)=>{
...
const sharedocument=()=>{
  axios.post(`${ipaddress}/GenerateTokenForDocumentDisplay/${id}/`)
  .then((r)=>{
...
const[copy_status,setcopy_status]=useState("")
const copyLinkToClipboard = (token) => {
  const linkText = `http://13.201.18.209/shareddocument/${token}/`;

  navigator.clipboard.writeText(linkText)
      .then(() => {
          console.log('Link copied to clipboard:', linkText);
          setcopy_status("Copied")
          setTimeout(()=>{
            changeCopy_status()
          },1000)
      })
...
const changeCopy_status=()=>{
  setcopy_status("")
}

// ---------------------------------------------Download pdf functionality----------------------------------------------------
const downloadPDF = (url) => {
  axios.post(`${ipaddress}/DownloadDocument/`,{
...
const unsave=()=>{
  axios.delete(`${ipaddress}/UserStudyListView/${user.user_id}/${id}/`)
  .then((r)=>{
...
axios.delete(`${ipaddress}/unpindocumentcomments/${user.user_id}/${id}/${discid}/`)
.then((r)=>{
  if(usercommentsstate==true){
    fetchuserComments()
  }
...
<div className="d-flex justify-content-between pb-2 align-items-center">
 ...
 {/* -----------------------------------------Rating button for document---------------------------------------------- */}
         ...
         data-bs-title="Rating">
          <i data-bs-toggle="modal" data-bs-target="#ratingmodal" class={pdfurldata.rating_status ? "fa-solid fa-star fs-5":"fa-regular fa-star fs-5"}></i>
 ...
 {/* ---------------------------------------View Extracted text button-------------------------------------------------- */}
         <span data-bs-toggle="tooltip" data-bs-placement="top"
                 ...
                 <svg xmlns="http://www.w3.org/2000/svg" width="25" height="29" fill="currentColor" class="bi bi-eye" viewBox="0 0 16 16">
...
{/* ---------------------------------------------Share Document---------------------------------------------------------- */}
          <button className="btn ms-3 p-0 border-0" onClick={()=>{
            sharedocument()
...
{/* ------------------------------------------------Report the Document--------------------------------------------------- */}
<span className={`ms-4 ps-2 ${pdfurldata.reported_status ? 'd-none':''}`} data-bs-toggle="modal" data-bs-target="#reportmodal" style={{cursor:'pointer',position:'relative',color:'#fff'}}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-exclamation-octagon" viewBox="0 0 16 16">
...
}} className={`ms-4 ps-2 ${pdfurldata.reported_status ? '':'d-none'}`} style={{cursor:'pointer',position:'relative',color:'#fff'}}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-exclamation-octagon" viewBox="0 0 16 16">
<path d="M4.54.146A.5.5 0 0 1 4.893 0h6.214a.5.5 0 0 1 .353.146l4.394 4.394a.5.5 0 0 1 .146.353v6.214a.5.5 0 0 1-.146.353l-4.394 4.394a.5.5 0 0 1-.353.146H4.893a.5.5 0 0 1-.353-.146L.146 11.46A.5.5 0 0 1 0 11.107V4.893a.5.5 0 0 1 .146-.353zM5.1 1 1 5.1v5.8L5.1 15h5.8l4.1-4.1V5.1L10.9 1z"/>
<path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0M7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0z"/>
</svg></span>
...
<Link to={`/extracted_text/${id}`} data-bs-toggle="tooltip" data-bs-placement="top"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="currentColor" class="bi bi-fonts" viewBox="0 0 16 16">
...
<div className="m-0 d-flex align-items-center mt-4 gap-lg-3">
 ...
 <p className='text-white m-0 mt-2'><span className="me-2"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-journal-check" viewBox="0 0 16 16">
  <path fill-rule="evenodd" d="M10.854 6.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 1 1 .708-.708L7.5 8.793l2.646-2.647a.5.5 0 0 1 .708 0"/>
  <path d="M3 0h10a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2v-1h1v1a1 1 0 0 0 1 1h10a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H3a1 1 0 0 0-1 1v1H1V2a2 2 0 0 1 2-2"/>
  <path d="M1 5v-.5a.5.5 0 0 1 1 0V5h.5a.5.5 0 0 1 0 1h-2a.5.5 0 0 1 0-1zm0 3v-.5a.5.5 0 0 1 1 0V8h.5a.5.5 0 0 1 0 1h-2a.5.5 0 0 1 0-1zm0 3v-.5a.5.5 0 0 1 1 0v.5h.5a.5.5 0 0 1 0 1h-2a.5.5 0 0 1 0-1z"/>
</svg></span>{!loading && !error && (
                   pdfurldata && (
...
<span className="d-none d-md-inline" data-bs-toggle="tooltip" data-bs-placement="top"
         data-bs-custom-class="custom-tooltip"
         data-bs-title="Pinned comments">
<button onClick={()=>{
   fetchpinnedComments()
  setusercommentsstate(false)
  setpinnedcommentsstate(!pinnedcommentsstate)
}} className="btn btn-sm ms-2 px-3 py-2" style={{border:'1px solid #445259',color:'#445259',height:'40px'}}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pin" viewBox="0 0 16 16">
...
<button onClick={()=>{
  ...
  fetchuserComments()
}} className="btn btn-sm ms-2 px-3 py-2" style={{border:'1px solid #445259',color:'#445259',height:'40px'}}><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
...
<button className="btn btn-sm ms-2 px-3 py-2 d-none d-md-inline" style={{border:'1px solid #445259',color:'#445259',height:'40px'}} data-bs-toggle="modal" data-bs-target="#geminiai"><i class="fa-solid fa-circle-question"></i></button>
</span>
         <span className="" data-bs-toggle="tooltip" data-bs-placement="top"
         data-bs-custom-class="custom-tooltip"
         data-bs-title="Translate the Document">
         <button className="btn btn-sm ms-2 px-3 py-2" data-bs-toggle="modal" data-bs-target="#translatemodal" style={{border:'1px solid #445259',color:'#445259'}}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-translate" viewBox="0 0 16 16">
...
<div style={{position:'sticky',height:'100vh'}} className={`${discussionpagelayout ? 'col-lg-12 pb-4':'col-lg-4'} ${documentpagelayout ? 'd-none':''} mt-3 mt-lg-0`} id="disc-div">
<div  style={{position:'relative'}}>
<div style={{width:discussionpagelayout ? '802px':'100%'}} id="btn" className="mx-auto">
  <span data-bs-toggle="tooltip" data-bs-placement="top"
         data-bs-custom-class="custom-tooltip"
         data-bs-title="Expand the Comment Section" className="border border-dark rounded-circle d-flex justify-content-center align-items-center" onClick={()=>{setdiscussionpagelayout(!discussionpagelayout)}} style={{height:'30px',width:'30px',position:'absolute',right:'-18px',top:'20px',cursor:'pointer'}}>
  ...
  <path d="M26.6154 22.2923L25.9077 23L20.6 17.6923L25.9077 12.3846L26.6154 13.0923L22.0154 17.6923L26.6154 22.2923ZM14.0154 17.6923L8.7077 23L8 22.2923L12.6 17.6923L8 13.0923L8.7077 12.3846L14.0154 17.6923Z" fill="#2A3941"/>
                   ...
                   {/* <button className="bg-transparent border-0" onClick={discussionAction}><i class="fa-solid fa-angle-down fa-rotate-270"></i></button> */}
                     <div className='bg-white rounded shadow-sm py-3 px-3' onClick={()=>{
              setindex1(-1)
             }}>
                     <div class="input-group bg-light rounded border" style={{cursor:'pointer'}}>
<span class="input-group-text bg-transparent border-0" id="basic-addon1"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
...
<input onChange={(e)=>{
 ...
 searchcomments(e.target.value)
                     }} type="text" class="form-control bg-transparent border-0 ps-0 sub-search" placeholder={translate_value.common_words.search_discussion} aria-label="Username" aria-describedby="basic-addon1" style={{fontSize:'14px'}}/>
</div>
     </div>


     <div className="mt-3 rounded" style={{paddingBottom:documentDetails.length>0 ? '80px':'10px',height:documentDetails.length>0 ? '100vh':'100px',overflowY:documentDetails.length>0 ? 'scroll':'none'}}>
     <div>
...
{searchedComments.map((x,index)=>{
      ...
      {x.user_id.nickname!=undefined ? (<p className={x.user_id.profile_pic ==null ? 'bg-info text-white rounded-circle my-auto d-flex align-items-center justify-content-center' : 'd-none'} style={{fontSize:'16px',height:'35px',width:'35px'}}><span>{x.user_id.nickname.slice(0,1)}</span><span>{x.user_id.nickname.slice(-1)}</span></p>):(<></>)} 
             ...
             <Link to={`/profile/${x.user_id.user_id}`} className='ms-3 ms-sm-0 my-0 fw-medium text-dark text-decoration-none' style={{fontSize:'14px'}}>{x.user_id.nickname}
                 ...
                 <p className='ms-3 ms-sm-0 my-0' style={{fontSize:'12px'}}>{x.created_on}</p>
             </div>
             ...
             <button onClick={()=>{
  unpin(x.ddpid)
 }} className={`p-1 btn btn-sm border px-2 ${x.pinned_status && discussionpagelayout ? '':'d-none'}`} style={{cursor:'pointer'}}><i class="fa-solid fa-link-slash d-md-none d-inline"></i><span className='d-none d-md-inline'>Followed</span></button>
            
 <button onClick={()=>{
  unpin(x.ddpid)
 }} className={`p-1 btn btn-sm p-0 px-2 me-2 border-0 ${x.pinned_status && discussionpagelayout==false ? '':'d-none'}`} style={{cursor:'pointer'}}> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pin" viewBox="0 0 16 16">
   <path d="M4.146.146A.5.5 0 0 1 4.5 0h7a.5.5 0 0 1 .5.5c0 .68-.342 1.174-.646 1.479-.126.125-.25.224-.354.298v4.431l.078.048c.203.127.476.314.751.555C12.36 7.775 13 8.527 13 9.5a.5.5 0 0 1-.5.5h-4v4.5c0 .276-.224 1.5-.5 1.5s-.5-1.224-.5-1.5V10h-4a.5.5 0 0 1-.5-.5c0-.973.64-1.725 1.17-2.189A6 6 0 0 1 5 6.708V2.277a3 3 0 0 1-.354-.298C4.342 1.674 4 1.179 4 .5a.5.5 0 0 1 .146-.354m1.58 1.408-.002-.001zm-.002-.001.002.001A.5.5 0 0 1 6 2v5a.5.5 0 0 1-.276.447h-.002l-.012.007-.054.03a5 5 0 0 0-.827.58c-.318.278-.585.596-.725.936h7.792c-.14-.34-.407-.658-.725-.936a5 5 0 0 0-.881-.61l-.012-.006h-.002A.5.5 0 0 1 10 7V2a.5.5 0 0 1 .295-.458 1.8 1.8 0 0 0 .351-.271c.08-.08.155-.17.214-.271H5.14q.091.15.214.271a1.8 1.8 0 0 0 .37.282"/>
 </svg></button>

             <div class="btn-group dropstart">
             <span class="border-0" type="button" style={{position:'relative'}} onClick={()=>{
...
{/* ---------------------------------------------Document discussion layout section-------------------------------------- */}
 <h6 className={`py-2 ${pinnedcommentsstate ? '':'d-none'}`} style={{color:'#5d5fe3',fontSize:'15px'}}>💬 Pinned Comments</h6>
...
{fetchedreplies_for_reply && fetchedreplies_for_reply.length > 0 && ( 
 ...
 <div className='col-12 bg-white px-3 pt-2'>
    ...
    {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{fontSize:'14px',height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
             <div class="input-group bg-light rounded border pe-3 py-1">
                   <input
                     type="text"
                     name="question"
                     ...
                     // value={question}
                     class="form-control border-0 bg-light py-2 ps-3" placeholder="Post your comment....."
                     style={{position:'relative'}}/>
  ...
  <path d="M21.6825 19.4231C21.6825 21.3164 21.0302 22.9276 19.7258 24.2565C18.4213 25.5855 16.8259 26.25 14.9397 26.25C13.0535 26.25 11.4541 25.5855 10.1416 24.2565C8.8291 22.9276 8.17285 21.3164 8.17285 19.4231V8.50962C8.17285 7.1875 8.62758 6.0637 9.53704 5.13822C10.4465 4.21274 11.5623 3.75 12.8844 3.75C14.2065 3.75 15.3223 4.21274 16.2318 5.13822C17.1412 6.0637 17.5959 7.1875 17.5959 8.50962V18.8462C17.5959 19.5801 17.3399 20.2123 16.8277 20.7428C16.3155 21.2732 15.6913 21.5385 14.9551 21.5385C14.2189 21.5385 13.5855 21.2774 13.0551 20.7552C12.5246 20.2331 12.2594 19.5967 12.2594 18.8462V8.46153H13.5094V18.8462C13.5094 19.2452 13.646 19.5853 13.9193 19.8666C14.1925 20.1478 14.5286 20.2885 14.9277 20.2885C15.3267 20.2885 15.6628 20.1478 15.9361 19.8666C16.2093 19.5853 16.3459 19.2452 16.3459 18.8462V8.48556C16.3411 7.51442 16.0072 6.69071 15.3443 6.01444C14.6814 5.33815 13.8614 5 12.8844 5C11.9152 5 11.0959 5.34215 10.4267 6.02644C9.75747 6.71073 9.42285 7.53846 9.42285 8.50962V19.4231C9.41806 20.9663 9.95252 22.2816 11.0262 23.369C12.1 24.4563 13.4073 25 14.9484 25C16.4677 25 17.7592 24.4563 18.8227 23.369C19.8863 22.2816 20.4229 20.9663 20.4325 19.4231V8.46153H21.6825V19.4231Z" fill="#8E9696"/>
                 ...
                 <button className="btn btn-sm"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
  <path d="M5 23.125V6.875L24.2789 15L5 23.125ZM6.25 21.25L21.0625 15L6.25 8.75V13.6058L12.3077 15L6.25 16.3942V21.25Z" fill="#8E9696"/>
           ...
           <button data-bs-dismiss="modal" onClick={postQuestion} className='border-0 text-white p-2 px-3 ms-1 ms-md-5 mt-2' style={{backgroundColor:'#5D5FE3',fontSize:'14px',fontWeight:500,borderRadius:'5px'}}>Post</button>
           <div className='d-flex gap-3 mt-3'>
           {selectedImage.length > 0 &&
          selectedImage.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              <img src={URL.createObjectURL(image)} width={50} alt={`Selected Image ${index + 1}`} />
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removemainImage(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
  <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
</svg></button>
            </div>
...
<span data-bs-toggle="tooltip" data-bs-placement="left"
         data-bs-custom-class="custom-tooltip"
         data-bs-title="Read the Pdf" className="border border-dark rounded-circle d-flex justify-content-center align-items-center" onClick={()=>{setdocumentpagelayout(!documentpagelayout)}} style={{height:'30px',width:'30px',cursor:'pointer',position:'absolute',right:'0px',top:'10px'}}><svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 35 35" fill="none">
         <circle cx="17.5" cy="17.5" r="17.25" fill="#F9F9FB" stroke="#2A3941" stroke-width="0.5"/>
         <path d="M7.60896 17.4711L13.1378 23L13.881 22.2568L9.13541 17.4711L13.881 12.6855L13.1378 11.9423L7.60896 17.4711ZM20.734 22.2508L21.4711 23L27 17.4711L21.4711 11.9423L20.734 12.6915L25.4735 17.4711L20.734 22.2508Z" fill="#2A3941"/>
       </svg></span>
...
{pdfurldata && pdfurldata.documnet_id && pdfurldata.documnet_id.document_url!=undefined ? (<button className="btn text-white btn-sm ms-4 border-0" data-bs-toggle="tooltip" data-bs-placement="top"
         data-bs-custom-class="custom-tooltip"
         data-bs-title="Download the Pdf" onClick={()=>{
   downloadPDF(pdfurldata.documnet_id.document_url)
}}>
<svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 35 35" fill="none">
...
<span style={{cursor:'pointer'}} data-bs-toggle="tooltip" data-bs-placement="top"
         ...
         <path d="M20.2482 13.9886V12.6312C21.0316 12.2348 21.8706 11.9376 22.7653 11.7394C23.6599 11.5412 24.5784 11.4421 25.5207 11.4421C26.0591 11.4421 26.5761 11.4767 27.0716 11.5459C27.567 11.615 28.0784 11.7132 28.6056 11.8403V13.1641C28.097 13.0014 27.6011 12.8888 27.1178 12.8261C26.6345 12.7635 26.1021 12.7322 25.5207 12.7322C24.5784 12.7322 23.6585 12.8383 22.7611 13.0505C21.8636 13.2627 21.026 13.5754 20.2482 13.9886ZM20.2482 21.9533V20.5399C20.9942 20.1435 21.8286 19.8462 22.7512 19.648C23.6739 19.4498 24.5971 19.3507 25.5207 19.3507C26.0591 19.3507 26.5761 19.3853 27.0716 19.4545C27.567 19.5237 28.0784 19.6218 28.6056 19.749V21.0727C28.097 20.9101 27.6011 20.7974 27.1178 20.7348C26.6345 20.6721 26.1021 20.6408 25.5207 20.6408C24.5784 20.6408 23.6585 20.7549 22.7611 20.983C21.8636 21.2111 21.026 21.5345 20.2482 21.9533ZM20.2482 17.999V16.5855C21.0316 16.1892 21.8706 15.8919 22.7653 15.6937C23.6599 15.4955 24.5784 15.3964 25.5207 15.3964C26.0591 15.3964 26.5761 15.431 27.0716 15.5002C27.567 15.5694 28.0784 15.6675 28.6056 15.7947V17.1184C28.097 16.9557 27.6011 16.8431 27.1178 16.7804C26.6345 16.7178 26.1021 16.6865 25.5207 16.6865C24.5784 16.6865 23.6585 16.8019 22.7611 17.0328C21.8636 17.2637 21.026 17.5858 20.2482 17.999ZM9.479 23.3892C10.7523 23.3892 11.9904 23.5355 13.1936 23.8281C14.3967 24.1207 15.5891 24.597 16.7707 25.257V10.9485C15.718 10.1969 14.5579 9.63322 13.2903 9.2574C12.0227 8.88161 10.7523 8.69372 9.479 8.69372C8.604 8.69372 7.8379 8.74606 7.18071 8.85075C6.52353 8.95546 5.79391 9.1499 4.99182 9.43408C4.76746 9.50887 4.60854 9.61637 4.51506 9.75659C4.42158 9.89683 4.37484 10.0511 4.37484 10.2194V23.3668C4.37484 23.6285 4.46832 23.8202 4.65527 23.9417C4.84226 24.0632 5.04792 24.0772 5.27226 23.9838C5.80511 23.8043 6.4207 23.6603 7.11902 23.5519C7.81734 23.4434 8.604 23.3892 9.479 23.3892ZM18.229 25.257C19.4106 24.597 20.603 24.1207 21.8061 23.8281C23.0092 23.5355 24.2474 23.3892 25.5207 23.3892C26.3957 23.3892 27.1823 23.4434 27.8807 23.5519C28.579 23.6603 29.1946 23.8043 29.7274 23.9838C29.9518 24.0772 30.1574 24.0632 30.3444 23.9417C30.5314 23.8202 30.6248 23.6285 30.6248 23.3668V10.2194C30.6248 10.0511 30.5781 9.90151 30.4846 9.77063C30.3911 9.63974 30.2322 9.52756 30.0079 9.43408C29.2058 9.1499 28.4761 8.95546 27.819 8.85075C27.1618 8.74606 26.3957 8.69372 25.5207 8.69372C24.2474 8.69372 22.977 8.88161 21.7094 9.2574C20.4418 9.63322 19.2816 10.1969 18.229 10.9485V25.257ZM17.4998 27.3716C16.3145 26.5602 15.0412 25.9366 13.6801 25.501C12.319 25.0654 10.9186 24.8476 9.479 24.8476C8.71992 24.8476 7.97439 24.9111 7.24243 25.0383C6.51045 25.1654 5.79764 25.3692 5.104 25.6497C4.5749 25.8609 4.07803 25.7973 3.6134 25.4589C3.1488 25.1205 2.9165 24.6531 2.9165 24.0567V9.96134C2.9165 9.60048 3.01138 9.2658 3.20113 8.95732C3.39091 8.64883 3.65688 8.43382 3.99902 8.31229C4.85533 7.93461 5.74623 7.6607 6.67171 7.49056C7.59719 7.32042 8.53296 7.23535 9.479 7.23535C10.9074 7.23535 12.3008 7.44101 13.6591 7.85233C15.0174 8.26366 16.2976 8.86195 17.4998 9.64722C18.702 8.86195 19.9823 8.26366 21.3406 7.85233C22.6989 7.44101 24.0923 7.23535 25.5207 7.23535C26.4667 7.23535 27.4025 7.32042 28.328 7.49056C29.2534 7.6607 30.1443 7.93461 31.0006 8.31229C31.3428 8.43382 31.6088 8.64883 31.7985 8.95732C31.9883 9.2658 32.0832 9.60048 32.0832 9.96134V24.0567C32.0832 24.6531 31.8322 25.1112 31.3302 25.4309C30.8282 25.7506 30.2939 25.8048 29.7274 25.5935C29.0525 25.3318 28.363 25.142 27.6591 25.0242C26.9552 24.9065 26.2423 24.8476 25.5207 24.8476C24.081 24.8476 22.6807 25.0654 21.3195 25.501C19.9584 25.9366 18.6852 26.5602 17.4998 27.3716Z" fill="#fff"/>
           ...
           {/* <div className="ms-4">
        <input className="rounded border border-white bg-transparent ps-3 text-white" style={{width:'50px'}}
          type="number"
          min={1}
          max={numPages || 1}
          value={pageNumber}
        ...
        />
        <span className="text-white ms-1" style={{fontSize:'14px'}}> of {numPages || '-'}</span>
      </div> */}
           {/* <button className="btn btn-sm ms-5 text-danger" onClick={handleSpeechButtonClick}><i class="fa-solid fa-volume-high"></i></button> */}
         </div>
</div>
       {loading && <p>Loading...</p>}
       {error && <p>{error}</p>}
       {!loading && !error && (
         ...
         <div id="pdf-container" className="border d-flex justify-content-center" onClick={handlePdfClick} style={{ position: 'relative', marginBottom: '20px',zIndex:'1px',width:'100%',overflowX:'scroll'}}>
           {pdfurldata && (
   ...
   <div
     style={{
       ...
       zIndex: 2,
       opacity: 0.2,
       background: 'blue', // Customize the highlighter color
     }}
   />
 )}
...
{/* -------------------------------------------TRANSLATE DOCUMENT TOAST MESSAGE---------------------------------------- */}
 <div class="toast-container position-fixed bottom-0 end-0 p-3">
   <div id="translateToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true">
   ...
   <div class="modal-dialog modal-dialog-centered modal-lg">
     <div class="modal-content">
       <div class="modal-body">
        ...
        <textarea className="w-100 mt-2 p-2 form-control" style={{fontSize:'14px',outline:'none'}} id="" cols="30" rows="2" value={report} onChange={reportData}></textarea>
        <div className="text-end mt-2">
         <button className="text-white btn btn-sm px-3" style={{backgroundColor:'#5d5fe3'}} data-bs-dismiss="modal" onClick={sendreport}>Send</button>
        </div>
         </div>
       </div>
     </div>
 ...
 <div className='bg-white px-3 pt-2 pb-3'>
  ...
  <div class="modal fade" id="sharedocumentlinkmodal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered modal-lg">
     <div class="modal-content">
       <div class="modal-body">
        ...
        <a target="_blank" href={`http://13.201.18.209/shareddocument/${token}/`}>{`http://13.201.18.209/shareddocument/${token}/`}</a>
         <button className="btn btn-sm d-flex align-items-center ms-4 border-0" onClick={()=>{
          copyLinkToClipboard(token)
      }}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-copy" viewBox="0 0 16 16">
  <path fill-rule="evenodd" d="M4 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2zm2-1a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1zM2 5a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-1h1v1a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h1v1z"/>
</svg> <span className="ms-1">{copy_status}</span></button>
        </div>
         </div>
       </div>
     </div>
...
<div class="modal fade" id="replyforreply_modal" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        ...
        {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
              <div class="input-group bg-light border rounded pe-3">
                  ...
                  <label
                    ...
                    <button disabled={isLoading} data-bs-dismiss="modal" onClick={postreply_for_replies} className='text-secondary h-100 bg-transparent border-0 ms-2 outline-0' ><span className={isLoading ? "spinner-border spinner-border-sm" : ""} aria-hidden="true"></span><i class={isLoading ? '' : "fa-solid fa-paper-plane"}></i></button>
                    </div>
                  </div>
          ...
          replies_reply_Image.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              <img src={URL.createObjectURL(image)} width={50} alt={`Selected Image ${index + 1}`} />
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removereply_Image(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
  <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
</svg></button>
            </div>
...
<div class="modal fade" id="markquestionModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-body">
        <p className="m-0 d-flex align-items-center justify-content-center" style={{color:'#5d5fe3'}}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="green" class="bi bi-check2-circle" viewBox="0 0 16 16">
 ...
 <Backtotop/>
<span className={`${isVisible ? 'd-none d-md-inline' : 'd-none'}`} data-bs-toggle="tooltip" data-bs-placement="left"
                 data-bs-custom-class="custom-tooltip"
                 data-bs-title="Create Post"  style={{position:'fixed',left:'96%',top:'82%'}}>
<button data-bs-toggle="modal" data-bs-target="#postmodal"  style={{color:'#5d5fe3'}}
