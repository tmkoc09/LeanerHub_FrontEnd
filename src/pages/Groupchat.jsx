...
const fetchdocs=()=>{
  setsubLoading(true)
  axios.get(`${ipaddress}/GroupDocuments/${user.user_id}/${group_id}/`)
  .then((r)=>{
...
const getallflashsetsundergroup=()=>{ 
  setsubLoading(true)  
  axios.get(`${ipaddress2}/GroupFlashCardSetRetrieveView/${group_id}/${user.user_id}/`)
      .then((r)=>{
...
const filterflashsets=(value)=>{
  if(value==="likes"){
  axios.post(`${ipaddress2}/FlashsetsFiltersGroups/${user.user_id}/${group_id}/`,{
    'like':true,
    'datetime':false})
  .then((r)=>{
    console.log("Liked")
    console.log("date")
    console.log("Group Flashsets Filtered Successfuly",r.data)
...
const postQuestion = async (e) => {
  ...
  if(question.length>0){
    try {
      const response = await fetch(`${ipaddress}/NewGroupDiscussionView/${group_id}/${user.user_id}/${question}/`, {
        method: 'POST',
        body: formData,
      });
...
const getdiscussion=()=>{
  const user=JSON.parse(sessionStorage.getItem('user'))
  axios.get(`${ipaddress}/NewGroupDiscussionView/${group_id}/${user.user_id}/`)
  .then((r)=>{
...
const reportGroup=()=>{
  ...
  axios.post(`${ipaddress}/ReportGroup/${group_id}/${user.user_id}/`,report)
  .then((r)=>{
...
const searchDocuments=(value)=>{
  if(value.length>0){
  axios.post(`${ipaddress}/SearchFlashcardsDocumentsUnderPrivateGroup/${user.user_id}/${group_id}/${value}/`)
  .then((r)=>{
...
const searchFlashsets=(value)=>{
  if(value.length>0){
  axios.get(`${ipaddress2}/SearchFlashsetsGroups/${user.user_id}/${group_id}/${value}/`)
  .then((r)=>{
...
const leaveGroup=()=>{
const user=JSON.parse(sessionStorage.getItem('user'))
axios.post(`${ipaddress}/exitGroup/${user.user_id}/${group_id}/`,{
...
axios.put(`${ipaddress}/makeadmin/${user_id}/${group_id}/`)
.then((r)=>{
...
const kickout=(user_id,nickname)=>{
  axios.delete(`${ipaddress}/kickout/${user_id}/${group_id}/`)
  .then((r)=>{
...
const unsave=(doc_id)=>{
  axios.delete(`${ipaddress}/UserStudyListView/${user.user_id}/${doc_id}/`)
  .then((r)=>{
...
const unsave_flashset=(flashset_id)=>{
  axios.put(`${ipaddress2}/Unsave_flashcard_studylist/${user.user_id}/${flashset_id}/`)
  .then((r)=>{
...
<div className='d-flex justify-content-between mb-3'>
  ...
  <div className=''>
<button className={`p-1 border-0 bg-transparent ${groupdetails.group_member_status ? '':'d-none'}`} data-bs-toggle="modal" data-bs-target="#exitgroupmodal"><svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="#fff" class="bi bi-box-arrow-left" viewBox="0 0 16 16">
...
<ul class={`${reportdropdownstate ? '':'d-none'} rounded me-5 bg-white border-0 py-2 mt-3 px-2 ps-3 report-dropdown`} style={{width:'180px',listStyleType:'none',position:'absolute',right:'120px',top:'150px'}}>
  <li className={`${groupdetails.group_report_status ? 'd-none':''}`}><a data-bs-toggle="modal" data-bs-target="#reportgroupmodal" class="dropdown-item d-flex align-items-center" style={{cursor:'pointer'}}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-octagon" viewBox="0 0 16 16">
<path d="M4.54.146A.5.5 0 0 1 4.893 0h6.214a.5.5 0 0 1 .353.146l4.394 4.394a.5.5 0 0 1 .146.353v6.214a.5.5 0 0 1-.146.353l-4.394 4.394a.5.5 0 0 1-.353.146H4.893a.5.5 0 0 1-.353-.146L.146 11.46A.5.5 0 0 1 0 11.107V4.893a.5.5 0 0 1 .146-.353zM5.1 1 1 5.1v5.8L5.1 15h5.8l4.1-4.1V5.1L10.9 1z"/>
<path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0M7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0z"/>
</svg> <span className='ms-2'>Report Irrelevant</span></a></li>
  <li className={`${groupdetails.group_report_status ? 'd-none':'mt-2'}`}><a data-bs-toggle="modal" data-bs-target="#reportgroupmodal" class="dropdown-item d-flex align-items-center" href="#"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bug" viewBox="0 0 16 16">
...
<li className={`${groupdetails.group_report_status ? '':'d-none'}`} style={{color:'#FF845D'}}><a class="dropdown-item d-flex align-items-center" href="#"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bug" viewBox="0 0 16 16">
<path d="M4.355.522a.5.5 0 0 1 .623.333l.291.956A5 5 0 0 1 8 1c1.007 0 1.946.298 2.731.811l.29-.956a.5.5 0 1 1 .957.29l-.41 1.352A5 5 0 0 1 13 6h.5a.5.5 0 0 0 .5-.5V5a.5.5 0 0 1 1 0v.5A1.5 1.5 0 0 1 13.5 7H13v1h1.5a.5.5 0 0 1 0 1H13v1h.5a1.5 1.5 0 0 1 1.5 1.5v.5a.5.5 0 1 1-1 0v-.5a.5.5 0 0 0-.5-.5H13a5 5 0 0 1-10 0h-.5a.5.5 0 0 0-.5.5v.5a.5.5 0 1 1-1 0v-.5A1.5 1.5 0 0 1 2.5 10H3V9H1.5a.5.5 0 0 1 0-1H3V7h-.5A1.5 1.5 0 0 1 1 5.5V5a.5.5 0 0 1 1 0v.5a.5.5 0 0 0 .5.5H3c0-1.364.547-2.601 1.432-3.503l-.41-1.352a.5.5 0 0 1 .333-.623M4 7v4a4 4 0 0 0 3.5 3.97V7zm4.5 0v7.97A4 4 0 0 0 12 11V7zM12 6a4 4 0 0 0-1.334-2.982A3.98 3.98 0 0 0 8 2a3.98 3.98 0 0 0-2.667 1.018A4 4 0 0 0 4 6z"/>
</svg> <span className='ms-2'>Already Reported</span></a></li>

{/* <li className={`${groupdetails.course_report_status ? '':''}`} style={{color:'#ff845d'}}><a class="dropdown-item d-flex align-items-center" href="#"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bug" viewBox="0 0 16 16">
<path d="M4.355.522a.5.5 0 0 1 .623.333l.291.956A5 5 0 0 1 8 1c1.007 0 1.946.298 2.731.811l.29-.956a.5.5 0 1 1 .957.29l-.41 1.352A5 5 0 0 1 13 6h.5a.5.5 0 0 0 .5-.5V5a.5.5 0 0 1 1 0v.5A1.5 1.5 0 0 1 13.5 7H13v1h1.5a.5.5 0 0 1 0 1H13v1h.5a1.5 1.5 0 0 1 1.5 1.5v.5a.5.5 0 1 1-1 0v-.5a.5.5 0 0 0-.5-.5H13a5 5 0 0 1-10 0h-.5a.5.5 0 0 0-.5.5v.5a.5.5 0 1 1-1 0v-.5A1.5 1.5 0 0 1 2.5 10H3V9H1.5a.5.5 0 0 1 0-1H3V7h-.5A1.5 1.5 0 0 1 1 5.5V5a.5.5 0 0 1 1 0v.5a.5.5 0 0 0 .5.5H3c0-1.364.547-2.601 1.432-3.503l-.41-1.352a.5.5 0 0 1 .333-.623M4 7v4a4 4 0 0 0 3.5 3.97V7zm4.5 0v7.97A4 4 0 0 0 12 11V7zM12 6a4 4 0 0 0-1.334-2.982A3.98 3.98 0 0 0 8 2a3.98 3.98 0 0 0-2.667 1.018A4 4 0 0 0 4 6z"/>
</svg> <span className='ms-2'>Already Reported</span></a></li> */}
</ul>
</div>

<span className={`ms-4 ps-2 ${groupdetails.group_member_status ? '':'d-none'}`}><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 35 35" fill="none">
...
<span className={`ms-4 ps-2 text-white ${grouptype==="privategroup" ? '':'d-none'}`} data-bs-toggle="tooltip" data-bs-placement="top"
        ...
        data-bs-title="Group Info" style={{cursor:'pointer'}}>
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" fill="currentColor" class="bi bi-arrows-fullscreen" viewBox="0 0 16 16">
...
<h5 onClick={()=>{
              setreportdropdownstate(false)
             }} className='fw-bold text-white d-flex align-items-center'><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class={`bi bi-unlock-fill ${grouptype==="opengroup" ? '':'d-none'}`} viewBox="0 0 16 16">
<path d="M11 1a2 2 0 0 0-2 2v4a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2h5V3a3 3 0 0 1 6 0v4a.5.5 0 0 1-1 0V3a2 2 0 0 0-2-2"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class={`bi bi-lock-fill ${grouptype==="opengroup" ? 'd-none':''}`} viewBox="0 0 16 16">
...
<span className='text-white'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-geo-alt" viewBox="0 0 16 16">
...
</svg> <span className='ms-2'>{groupdetails.group_member_status ? 'Joined':'Join Group'}</span></Link>
              <button onClick={()=>{
...
<div className={`m-0 mt-3 mb-5 row justify-content-center`}>
            ...
            setreportdropdownstate(false)
                        ...
                        <ul class="nav nav-underline gap-1 w-md-50" id="myTab" role="tablist">
  ...
  <li class={`nav-item ${grouptype==="opengroup" ? 'd-none':''}`} role="presentation">
   {groupdocuments && (
     <button class="nav-link p-2 bg-white fw-normal" id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="profile-tab-pane" aria-selected="false" style={{color:tablist ==="documents" ? '#5D5FE3':'#8E9696',fontSize:'16px',letterSpacing:'0.36px',lineHeight:'normal'}} onClick={()=>{
  ...
  <li class={`nav-item ${grouptype==="opengroup" ? 'd-none':''}`} role="presentation">
    <button class="nav-link p-2 bg-white fw-normal" id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false" style={{color:tablist ==="flashcards" ? '#5D5FE3':'#8E9696',fontSize:'16px',letterSpacing:'0.36px',lineHeight:'normal'}}
    onClick={()=>{
...
<div class={`input-group bg-light rounded border ${tablist==="documents" ? '':'d-none'}`}>
  ...
  <input type="text" class="form-control bg-transparent border-0 ps-0 sub-search" placeholder={translate_value.common_words.group_search} onChange={(e)=>{
    searchDocuments(e.target.value)
  }} aria-label="Username" aria-describedby="basic-addon1" style={{fontSize:'14px'}}/>
</div>

{/* --------------------------Flashset search bar */}
<div class={`input-group bg-light rounded border ${tablist==="flashcards" ? '':'d-none'}`}>
  <span class="input-group-text bg-transparent border-0" id="basic-addon1"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
  <path d="M20.2965 20.9941L13.774 14.4717C13.2532 14.915 12.6542 15.2582 11.9771 15.5013C11.3001 15.7443 10.6196 15.8659 9.93586 15.8659C8.26746 15.8659 6.85544 15.2884 5.6998 14.1333C4.54414 12.9783 3.96631 11.5671 3.96631 9.89957C3.96631 8.23207 4.54382 6.81973 5.69883 5.66254C6.85385 4.50535 8.26511 3.92676 9.93261 3.92676C11.6001 3.92676 13.0125 4.50458 14.1696 5.66022C15.3268 6.81586 15.9054 8.2279 15.9054 9.89632C15.9054 10.6201 15.7772 11.3206 15.5208 11.9977C15.2644 12.6748 14.9279 13.2537 14.5112 13.7345L21.0336 20.2569L20.2965 20.9941ZM9.93586 14.8242C11.3181 14.8242 12.485 14.3484 13.4365 13.3969C14.388 12.4454 14.8638 11.2785 14.8638 9.89632C14.8638 8.51411 14.388 7.34725 13.4365 6.39572C12.485 5.44419 11.3181 4.96842 9.93586 4.96842C8.55366 4.96842 7.38679 5.44419 6.43527 6.39572C5.48376 7.34725 5.008 8.51411 5.008 9.89632C5.008 11.2785 5.48376 12.4454 6.43527 13.3969C7.38679 14.3484 8.55366 14.8242 9.93586 14.8242Z" fill="#8E9696"/>
</svg></span>
  <input type="text" class="form-control bg-transparent border-0 ps-0 sub-search" placeholder={translate_value.common_words.group_flashset} onChange={(e)=>{
...
<span className='d-flex align-items-center'>
  ...
  <span class="input-group-text bg-transparent border-0" id="basic-addon1"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
...
<div class="modal fade" id="postmodalform" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
      ...
      <div className='col-12 bg-white px-3 pt-2 pb-3'>
        ...
        {userdetails.nickname!=undefined ? (<p className={userdetails.profile_pic ==null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{userdetails.nickname.slice(0,1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>):(<></>)}
            <div class="input-group bg-light border rounded pe-3">
                  ...
                  {/* <button class="border-0 text-secondary bg-transparent h-100 d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#discussionimageModal" style={{padding:'12px'}}>
          ...
          selectedImages.map((image, index) => (
            <div key={index} className="image-preview bg-light p-2" style={{position:'relative'}}>
              ...
              <button style={{position:'absolute',top:'-10px',right:'-16px'}} className='btn btn-sm' onClick={() => removemain_image(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle" viewBox="0 0 16 16">
...
{groupdiscussion.map((x,index)=>{
  ...
  {x.sender.nickname && (<p className={x.sender.profile_pic ==null ? 'd-flex align-items-center justify-content-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{height:'40px',width:'40px'}}><span>{x.sender.nickname.slice(0,1)}</span><span>{x.sender.nickname.slice(-1)}</span></p>)}
          ...
          <div className={`col-2 ${x.pinned_status ? 'col-3':'col-2'} d-flex justify-content-between align-items-center p-0 ${groupdetails.group_member_status ? '':'d-none'}`} style={{position:'relative'}}>
          <button onClick={()=>{
            unpin(x.id)
  ...
  <path d="M26.2536 31.3543C25.2418 31.3543 24.3807 30.9999 23.6702 30.2909C22.9597 29.5821 22.6045 28.7212 22.6045 27.7085C22.6045 27.5146 22.6213 27.3137 22.655 27.1059C22.6886 26.8981 22.7391 26.7026 22.8064 26.5194L11.5155 19.8671C11.1584 20.2691 10.7434 20.5828 10.2703 20.8081C9.79734 21.0334 9.29066 21.146 8.75033 21.146C7.73759 21.146 6.87677 20.7919 6.16787 20.0836C5.45895 19.3753 5.10449 18.5153 5.10449 17.5034C5.10449 16.4916 5.45895 15.6305 6.16787 14.92C6.87677 14.2096 7.73759 13.8543 8.75033 13.8543C9.29066 13.8543 9.79734 13.967 10.2703 14.1923C10.7434 14.4175 11.1584 14.7312 11.5155 15.1332L22.8064 8.48092C22.7391 8.2977 22.6886 8.1022 22.655 7.89441C22.6213 7.68663 22.6045 7.48576 22.6045 7.29183C22.6045 6.27909 22.9586 5.41827 23.6669 4.70938C24.3752 4.00046 25.2352 3.646 26.247 3.646C27.2589 3.646 28.12 4.00013 28.8304 4.70839C29.5409 5.41668 29.8962 6.27672 29.8962 7.28851C29.8962 8.30035 29.5417 9.1615 28.8328 9.87195C28.1239 10.5824 27.2631 10.9377 26.2503 10.9377C25.71 10.9377 25.2033 10.825 24.7303 10.5997C24.2573 10.3744 23.8422 10.0608 23.4851 9.65881L12.1943 16.3111C12.2616 16.4943 12.312 16.6891 12.3457 16.8956C12.3793 17.1021 12.3962 17.3016 12.3962 17.4944C12.3962 17.6871 12.3793 17.8886 12.3457 18.0989C12.312 18.3093 12.2616 18.506 12.1943 18.6892L23.4851 25.3415C23.8422 24.9395 24.2573 24.6259 24.7303 24.4006C25.2033 24.1753 25.71 24.0627 26.2503 24.0627C27.2631 24.0627 28.1239 24.4168 28.8328 25.1251C29.5417 25.8333 29.8962 26.6934 29.8962 27.7052C29.8962 28.717 29.542 29.5782 28.8338 30.2886C28.1255 30.9991 27.2654 31.3543 26.2536 31.3543ZM26.2503 9.47933C26.8505 9.47933 27.3651 9.26478 27.7942 8.83569C28.2233 8.4066 28.4378 7.89198 28.4378 7.29183C28.4378 6.69168 28.2233 6.17705 27.7942 5.74796C27.3651 5.31887 26.8505 5.10433 26.2503 5.10433C25.6502 5.10433 25.1356 5.31887 24.7065 5.74796C24.2774 6.17705 24.0628 6.69168 24.0628 7.29183C24.0628 7.89198 24.2774 8.4066 24.7065 8.83569C25.1356 9.26478 25.6502 9.47933 26.2503 9.47933ZM8.75033 19.6877C9.35048 19.6877 9.8651 19.4731 10.2942 19.044C10.7233 18.6149 10.9378 18.1003 10.9378 17.5002C10.9378 16.9 10.7233 16.3854 10.2942 15.9563C9.8651 15.5272 9.35048 15.3127 8.75033 15.3127C8.15017 15.3127 7.63555 15.5272 7.20646 15.9563C6.77737 16.3854 6.56283 16.9 6.56283 17.5002C6.56283 18.1003 6.77737 18.6149 7.20646 19.044C7.63555 19.4731 8.15017 19.6877 8.75033 19.6877ZM26.2503 29.896C26.8505 29.896 27.3651 29.6814 27.7942 29.2524C28.2233 28.8233 28.4378 28.3086 28.4378 27.7085C28.4378 27.1083 28.2233 26.5937 27.7942 26.1646C27.3651 25.7355 26.8505 25.521 26.2503 25.521C25.6502 25.521 25.1356 25.7355 24.7065 26.1646C24.2774 26.5937 24.0628 27.1083 24.0628 27.7085C24.0628 28.3086 24.2774 28.8233 24.7065 29.2524C25.1356 29.6814 25.6502 29.896 26.2503 29.896Z" fill="#2a3941"/>
              ...
              <span class={`border-0 ${groupdetails.group_member_status ? '':'d-none'}`} type="button" style={{cursor:'pointer'}} onClick={()=>{
                if(index1==index)
                setindex1(-1)
              else
                setindex1(index)
...
<button className={`bg-transparent border-0 ${groupdetails.group_member_status ? 'd-none':''}`} style={{height:'20px',color: y.like_status ? "#ff845d" : "gray" }}><i class={`${y.like_status ? 'fa-solid fa-thumbs-up me-1 fs-5':'fa-regular fa-thumbs-up me-1 fs-5'}`}></i>{y.likes_count}</button>
...
<div class="modal fade" id="groupdescriptionmodal" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-body rounded bg-light py-4">
...
<div class="modal fade" id="exitgroupmodal" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  ...
  <div class="modal-content">
    <div class="modal-body pt-4 pb-3">
      ...
      <div className='mt-3 text-end'>
        <button className='btn btn-sm rounded-pill text-white fw-medium px-4' style={{backgroundColor:'#5d5fe3'}} data-bs-dismiss="modal" onClick={leaveGroup}>Send</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="reportgroupmodal" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
<div class="modal-dialog modal-md">
  <div class="modal-content">
    <div class="modal-body pt-4 pb-3">
      <div>
      ...
      <div className='mt-3 text-end'>
        <button className='btn btn-sm rounded-pill text-white fw-medium px-4' style={{backgroundColor:'#5d5fe3'}} data-bs-dismiss="modal" onClick={reportGroup}>Send</button>
      </div>
    </div>
  </div>
</div>
